...
- b7122c0: feat(DST-1213)!: migrate `@marigold/icons` to a `lucide-react` proxy with 13 custom icons.
  ...
  **BREAKING CHANGE**: All custom icons without a Lucide equivalent remain; icons with a Lucide equivalent have been removed. Consumers should import equivalents from the re-exported `lucide-react`, available through the same package:
  ...
  ```tsx
  import { DesignTicket, Search, Stadium } from '@marigold/icons';
  ```
...
- a5678ed: feat(DST-1213)!: migrate `@marigold/icons` to a `lucide-react` proxy with 13 custom icons.
