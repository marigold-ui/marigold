...
const activate = async (page: Page, trigger: Trigger): Promise<void> => {
  ...
  try {
    ...
    await handle.focus({ timeout: 1000 }).catch(() => {});
    if (trigger.activation === 'hover') {
      await handle.hover({ timeout: 1000 }).catch(() => {});
    } else {
      // Enter is the keyboard activation; fall back to a real click for
      // elements that only bind onClick.
      await page.keyboard.press('Enter').catch(() => {});
      await waitForLayout(page);
      const stillClosed = (await visibleOverlays(page)).length === 0;
      if (stillClosed) await handle.click({ timeout: 1000 }).catch(() => {});
    }
  } finally {
