...
| `theme-variants.ts`        | Checks prop values that are technically valid per TypeScript (an open string type) but don't correspond to a real theme variant (e.g. `size="huge"` when the theme only has `sm`/`md`/`lg`). Only runs if the theme package can actually be found on disk — otherwise the check is skipped entirely (no error). | `dist/appearances.cjs` in the installed `@marigold/theme-rui` (its own build output, published as the `@marigold/theme-rui/appearances` subpath — unlike its `*.styles.ts` sources, this ships in a real install)                                                                                                                               |
