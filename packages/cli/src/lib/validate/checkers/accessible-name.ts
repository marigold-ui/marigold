...
// A spread (`{...props}`) may carry aria-label/aria-labelledby, which cannot be
// resolved statically — so an overlay with a spread is treated as possibly
// labeled and left alone, to avoid a false positive.
const hasSpreadAttr = (attrs: ts.JsxAttributes): boolean =>
  attrs.properties.some(ts.isJsxSpreadAttribute);

// A child whose content is an opaque expression — `{children}`,
// `{renderBody()}`, `{items.map(...)}` — could carry the title at runtime, so
// the absence of a static `<X.Title>` is not provable. Inline render functions
// (`{() => <X.Title/>}`) are NOT opaque: their body is part of the AST and is
// searched normally.
      ...
      if (ts.isIdentifier(tag)) {
        ...
        if (overlay) {
          const named =
            ...
            hasSpreadAttr(node.openingElement.attributes);
