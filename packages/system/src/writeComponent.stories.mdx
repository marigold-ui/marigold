...
Next, `useStyles` takes a `variant` which is retrieved from the theme (which is given in the `<MarigoldProvider>`). Additionally, you can insert custom styles
...
import { MarigoldProvider, system } from '@marigold/system';
