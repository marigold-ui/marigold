...
test('popover is small screen', () => {
  ...
  expect(popover.className).toMatchInlineSnapshot(
    `"fixed! top-auto! bottom-0! left-0! max-h-fit! w-full group/popover z-50 outline-0 placement-top:mb-1 placement-bottom:mt-1 placement-right:ml-1 placement-left:mr-1 min-w-(--trigger-width)"`
  );
  expect(popover).toBeInTheDocument();
});
