...
/**
 * Returns how many action buttons fit; the rest collapse into the "More" menu.
 *
 * Widths come from a hidden layer that renders every control at its natural
 * width — measuring the live (squished) bar would under-report and collapse too
 * late. Two resize observers recalc (each fires on mount): the toolbar for the
 * available width, the hidden layer (`w-max`) for the set of controls. The
 * pinned controls' widths are reserved first, so only the actions collapse.
 */
