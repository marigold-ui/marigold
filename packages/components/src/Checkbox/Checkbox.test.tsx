...
test('check if all slot class names are applied correctly', () => {
  ...
  expect(getVisibleCheckbox()?.className).toMatchInlineSnapshot(
    `"grow-0 basis-4 items-center justify-center p-px bg-white border-solid grid size-4 shrink-0 place-content-center rounded border border-border shadow-elevation-border group-focus-visible/checkbox:ui-state-focus group-focus-visible/checkbox:border-(--ui-border-color) outline-none group-disabled/checkbox:group-selected/checkbox:bg-disabled-surface group-disabled/checkbox:border-disabled-surface! group-disabled/checkbox:text-disabled group-disabled/checkbox:cursor-not-allowed group-selected/checkbox:border-primary group-selected/checkbox:bg-primary group-selected/checkbox:text-primary-foreground group-[indeterminate]/checkbox:border-primary group-[indeterminate]/checkbox:bg-primary group-[indeterminate]/checkbox:text-primary-foreground group-hover/checkbox:group-disabled/checkbox:bg-disabled-surface"`
  );
