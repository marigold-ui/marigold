...
- **The caller is pseudonymous, not anonymous.** `hashCallerId` runs the Keycloak `sub` claim through HMAC-SHA256 keyed on `MCP_TELEMETRY_HASH_SECRET` before it leaves the process; the raw claim identifies an individual Reservix employee and is never written anywhere. But the digest is _stable_ for as long as the secret is — that's what makes unique-caller counts possible at all, and it also means the store holds a timestamped, per-person record of which doc pages that pseudonym searched. Anyone holding both Redis read access and the secret can re-identify a known `sub`. Treat the secret as a credential, rotate it to break linkability across periods, and see the retention note below.
...
**Events are retained indefinitely, and the bound's absence is deliberate.** `record.ts` sets a TTL on the rate-limit keys and none on the `telemetry:events` stream. Three reasons, in order of weight. Long-run adoption trends are the point of collecting this at all, and a window destroys them permanently — keeping data preserves the option to trim later, trimming destroys the option to keep. Volume makes it a non-issue: measured at roughly 35 events a day at ~250 bytes each, that is about 3 MB a year, so a decade costs less than a single embeddings file. And an unbounded stream costs nothing to read, because Insights only ever asks for a window (`XRANGE` seeks by stream id) rather than the whole thing.

This reverses the 90-day TTL that DST-1475 put on the old daily lists as a "storage leak" fix, and it applies to CLI telemetry too now that both sources share the stream. That earlier concern was a different shape: an unbounded number of _keys_, one per day, none of them read by anything. This is one key, with measured growth and an actual consumer. The reasoning is recorded in [`.memory/adr/0006-telemetry-retention.md`](../../../.memory/adr/0006-telemetry-retention.md) so it stops being re-decided.

Worth being explicit about what the two event types are, because they are not the same kind of data and only one of them is why retention is a question at all. `cli_command` carries `anonymousId`, a UUID minted locally by `crypto.randomUUID()` and tied to no identity — there is no personal data in it to retain. `mcp_tool_call` carries `hashedCallerId`, derived from a Keycloak `sub`, which is pseudonymous rather than anonymous: holding both Redis read access and `MCP_TELEMETRY_HASH_SECRET` re-identifies a named Reservix employee. Keeping that indefinitely is a deliberate call, taken on the basis that `marigold-docs` is an internal tool used by Reservix employees and that what is recorded is which doc pages were searched. There is no per-caller opt-out on the MCP path, unlike the CLI's `DO_NOT_TRACK` / `marigold telemetry disable`. If this store ever holds data about anyone outside Reservix, both of those positions need revisiting.
...
Both sources write to `telemetry:events`, discriminated on `event`, but only one of them is read. Insights' read-side schema is `z.literal('mcp_tool_call')`, so `cli_command` entries fail it and are dropped ([`dashboard/src/lib/telemetry.ts`](https://github.com/marigold-ui/insights/blob/main/dashboard/src/lib/telemetry.ts)). **CLI telemetry has had no consumer since DST-1264 introduced it** — it is collected, and now kept indefinitely, without anything reading it. Worth knowing before citing "we have CLI usage data".

That has one consequence that is easy to miss. Insights pages the stream at `STREAM_PAGE_SIZE = 5_000` with `MAX_STREAM_PAGES = 20`, and the page counter counts every entry in the window — including the `cli_command` ones it is about to discard. Past 100000 entries in a window it logs and returns **incomplete aggregates**, which look like a drop in usage rather than an error. At the measured ~35 events a day across both sources the window holds ~6000, so there is roughly 16x headroom; the point is that the headroom is consumed by the source nobody reads, and CLI volume is the one that grows with adoption. If it ever gets close, the fix is one stream per source, not a bigger page budget.

The two event shapes are also not symmetric, deliberately but worth stating: the CLI reports its outcome as `exitCode` and its duration as a coarse `durationBucket` (its public docs promise the bucket, not a timing), while MCP reports `success` and an exact `latencyMs`. Anything aggregating across both has to special-case, which is a third reason the shared stream is a storage decision rather than a common data model.
...
- [`record.ts`](../api/telemetry/record.ts) — rate limiting and the Redis write. `route.ts` calls this **in-process**; the server never POSTs to itself. The per-caller quota is keyed `cli:{anonymousId}` or `mcp:{hashedCallerId}`, at the two ceilings described above. A caller past its ceiling is dropped, not truncated, so both sit far above realistic usage — but it does mean a caller crossing one in a UTC day would under-report.
