...
- **The caller is pseudonymous, not anonymous.** `hashCallerId` runs the Keycloak `sub` claim through HMAC-SHA256 keyed on `MCP_TELEMETRY_HASH_SECRET` before it leaves the process; the raw claim identifies an individual Reservix employee and is never written anywhere. The digest is salted with the calendar quarter, recorded alongside it as `hashPeriod`: stable within a period, so unique-caller counts work over the window Insights reads, and unlinkable across periods without anyone having to act. Within a period the store still holds a per-person record of which doc pages that pseudonym searched, and anyone holding both Redis read access and the secret can re-identify a known `sub` for that period — so the secret is still a credential. Rotating it invalidates every past digest at once, which is the bigger hammer if one is ever needed. See the retention note below.
