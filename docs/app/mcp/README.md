...
- `Auth`, `Embed`, and `Record` are configured from Vercel project env vars, shown on the boxes above. `Emit` needs none: the caller digest is an unkeyed hash. See [Auth](#auth) for what each one does.
...
Everything above lives in [`route.ts`](./route.ts) except the two external calls, `Record`, and two small helpers it composes from: the memoisation on `Auth`/`Embed`/`Search` is [`lib/lazy.ts`](../../lib/lazy.ts) and `Emit`'s once-per-instance logging is [`lib/warn-once.ts`](../../lib/warn-once.ts). References below are by symbol name rather than line number, since line numbers go stale the moment anything above them shifts:
...
- **Search.** A brute-force dot-product top-K scan over the in-memory vector store (`search`). There is no approximate-nearest-neighbor index. The corpus is small enough — the ~160 pages under [`content/`](../../content), a few chunks each — that a linear scan is fast enough per request.
...
- [`etl/embedder.ts`](../../lib/markdown/etl/embedder.ts) embeds each chunk via AWS Bedrock (Titan v2, 512 dims, `eu-central-1`, see [`etl/config.ts`](../../lib/markdown/etl/config.ts)). It is rate-limited to a 280k-token/min budget with 5-way concurrency by default (`CONCURRENCY`), then uploads the result to Vercel Blob.
...
Env vars (all optional at build time so `pnpm build` works locally without secrets, see [#5374](https://github.com/marigold-ui/marigold/pull/5374)). The `OIDC_*` and `AWS_BEDROCK_*` ones are required at request time; the rest degrade rather than fail:
...
All seven vars above live in Vercel (see [Deployment](#deployment)). At request time a `search_docs` call only needs the `OIDC_*` and `AWS_BEDROCK_*` vars — `BLOB_READ_WRITE_TOKEN` is build-time only (see [Data pipeline](#data-pipeline)), and the two `KV_REST_API_*` vars only turn recording on or off. Recording needs no secret of its own — the caller digest is an unkeyed SHA-256 (see [Telemetry](#telemetry)).
...
- **The caller is pseudonymous, not anonymous.** `hashCallerId` runs the Keycloak `sub` claim through SHA-256 before it leaves the process; the raw claim identifies an individual Reservix employee and is never written anywhere. The hash is **unkeyed, deliberately**: a Keycloak `sub` is a UUID by default, so there is no candidate space to enumerate, and a key would only raise the bar against someone already holding `sub`s to test against. That property depends on realm configuration this repo neither asserts nor checks — see [the telemetry README](../api/telemetry/README.md#two-event-types-one-store) for what a key would have bought and what dropping it gave up. The digest is **stable for good** — that is what lets unique callers be counted over any window, including all of it, which is the point of keeping history indefinitely. It also means the store holds a permanently linkable per-person record of which doc pages that pseudonym searched, re-identifiable by anyone holding both Redis read access and a list of `sub`s to test against. What limits the exposure is not the hash but the read side: nothing counts a person, only distinct people. See [the telemetry README](../api/telemetry/README.md#two-event-types-one-store).
- **The write cannot affect the response.** `emitTelemetry` builds the event, then hands the Redis write to Next's [`after()`](https://nextjs.org/docs/app/api-reference/functions/after), which runs it once the response is already on its way out. The event is built _before_ `after()` is called, not inside its callback, so `latencyMs` measures the embed+search and not whenever the deferred callback happened to run. The whole body is wrapped in a `try`/`catch`, which matters because it is also called from the handler's own error path: a throw escaping it would throw a second time with no handler left. That includes a synchronous throw from `after()` itself, which is what Next does when there is no request scope.
...
The server ships as part of the `@marigold/docs` Next.js app on Vercel. `pnpm build` / `pnpm start` for this app are the MCP server's build and start. There's no dedicated `mcp:build` or `mcp:start` script. Aside from the Next.js app itself, Vercel is where everything else related to this server lives too. The Blob store holding `embeddings.json`, the Bedrock access keys, and the Keycloak/OIDC config (`OIDC_AUTHORITY`, `OIDC_CLIENT_ID`) are all Vercel project environment variables.

Check which project you are setting them on. This repo is wired to more than one Vercel project: branch previews build in `marigold-docs`, while `www.marigold-ui.io` — the production URL below — is served by `marigold-production`, which skips PR builds via an Ignored Build Step and builds from the `docs` branch CI pushes on release (see [`release.yml`](../../../.github/workflows/release.yml)). Every var above therefore has to exist on the project serving the production domain, not only where previews run. Set one only on the preview side and it works on every preview while being silently absent in production — for `KV_REST_API_*` the sole symptom is that Insights reads zero.
...
| How often a repeated telemetry failure is logged              | [`lib/warn-once.ts`](../../lib/warn-once.ts)                                                                           |
| How the JWKS set, Bedrock client and vector store memoize     | [`lib/lazy.ts`](../../lib/lazy.ts)                                                                                     |
