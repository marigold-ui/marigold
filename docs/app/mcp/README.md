...
Check which project you are setting them on. CI pushes a `docs` branch that the production build comes from (see [`release.yml`](../../../.github/workflows/release.yml)), while branch previews build separately, so the vars have to exist on whatever project serves `www.marigold-ui.io` — not only where previews run. A var set only on the preview side works on every preview and is silently absent in production, and for `KV_REST_API_*` the sole symptom is that Insights reads zero.
