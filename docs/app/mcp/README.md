...
**Retention is currently unbounded.** `record.ts` sets a TTL on the rate-limit keys but not on the `telemetry:YYYY-MM-DD` data lists, so events accumulate indefinitely — inherited from the CLI telemetry that predates this, but it matters more now that entries are keyed to a stable per-employee pseudonym. Purging old daily keys is a deliberate open item rather than an oversight: the retention window is a data-protection decision, and dropping keys silently would also delete history the Insights dashboards read. Agree a window with the DPO, then enforce it here.
