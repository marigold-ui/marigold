...
We don't explicit need to use a `onChange` event, we set a `defaultValue` for each field we want to submit in the `useForm` hook. With that the component is a [controlled component](https://www.joshwcomeau.com/react/data-binding/#introduction-to-controlled-fields-1), it never gets undefined or null.

There are `rules` that can be applied on the components to trigger the validation, e.g. `required: true`. Our Marigold form componets already have error properties and error messages build in. Now it should be combined.
