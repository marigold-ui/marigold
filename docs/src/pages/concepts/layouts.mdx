...
Although this gives app developers incredible freedom, it will also leave the burden on them to research and implement necessary layout patterns, while at the same time make them reusable and responsive. Ever heard of the [Flexbox Holy Albatross](https://heydonworks.com/article/the-flexbox-holy-albatross/)? Me neither.
...
To avoid unexpected behavior and layout shifts, Marigold follows these the principles. Managing whitespace, as welle as positioning and arranging components is reserved for a dedicated set of layout components. This approach ensures that components are as composable as possible while keeping white space predictable.
