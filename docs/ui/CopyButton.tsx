...
export const CopyButton = ({ codeString, variant }: CopyProps) => {
  const [isCopied, setCopied] = useClipboard(codeString, {
    successDuration: 1000,
  });
  const classNames = useClassNames({ component: 'Button', variant });

  return (
    <Button
      onPress={setCopied}
      className={cn('border-none p-0 outline-0 first:fill-white', classNames)}
    >
      {isCopied ? (
        <svg className="size-5" width="48" height="48" viewBox="0 0 24 24">
          <path d="M8.17368 16.6154L3.19528 11.637L1.5 13.3204L8.17368 19.994L22.5 5.66772L20.8167 3.98437L8.17368 16.6154Z"></path>
        </svg>
      ) : (
        <svg className="size-5" viewBox="0 0 16 16" fill="text.regular">
          <path
