...
If needed we add `default` values for the styles. Behind the `cva` statement are the styles that are always applied, in this example text color (which is applied for all variants). But sometimes we need only a default, that is not on all styles.
