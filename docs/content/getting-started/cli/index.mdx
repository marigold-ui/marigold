...
When components render unstyled or throw, run `marigold doctor --format json` to diagnose the project's setup (packages, versions, provider, theme, Tailwind, React peer deps) and apply the returned `suggestion` for each issue:
...
3. **Up to date**: installed versions are the latest published (best-effort, skipped when offline).
4. **MarigoldProvider wraps the app**: the root layout wraps the app in `<MarigoldProvider>`.
...
<Callout title="Safe for CI">
  ...
  warning. `marigold doctor` exits `1` only when there is an error. `--format
  json` emits `{ errors, warnings, passed, text }`, so an agent can derive
...
<Callout type="warning" title="PowerShell not supported">
...
`args` keeps only flag presence and values from a closed set. Closed-set flags (`--format`, `--section`, and `--limit` as a positive integer) are recorded as their validated value, or as `invalid` if unrecognised, so a typo is never echoed back. Identifier-shaped positionals (a component name, an example slug, a `--category`) are recorded only when they look like an identifier. A file path, absolute or relative, is recorded as `invalid`. **Free-text is never sent**: a `marigold search` query or a `list --search` term is recorded as `used`, so we learn that search was used but not what you searched for.
...
- Your IP address is never written into a telemetry record. It's necessarily visible to our host in transit, as with any HTTP request.

Events are kept indefinitely, to support long-run usage trends. That is a decision about volume, not about you: with no identifier in a record and timestamps rounded to the hour, a five-year-old event says no more about you than a five-minute-old one.
...
<Callout type="warning" title="If this ever changes">
  This reasoning holds only as long as the payload stays identifier-free. If a
  future version needs to link events to a person or a machine, the default has
  to flip to opt-in rather than being disclosed after the fact.
</Callout>
...
`DO_NOT_TRACK` follows the [consoledonottrack.com](https://consoledonottrack.com) convention, so setting it once opts you out of every tool that honours it. Telemetry is also suppressed automatically, with no configuration, when CI is detected (via [`ci-info`](https://github.com/watson/ci-info), covering GitHub Actions, GitLab CI, CircleCI, etc.).
