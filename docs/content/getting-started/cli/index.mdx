...
When you omit `--format`, the CLI picks one based on where its output goes:
...
An explicit `--format` always wins, so `marigold list --format plain | grep Date` still prints plain text.

## Usage with AI agents

AI coding agents are the CLI's primary audience. Agents capture the CLI's output instead of showing it in a terminal, so they get JSON by default (see [Output format](#output-format)). Pair it with `--section props` for a structured, precise payload. Passing `--format json` explicitly still works and keeps a command's output stable wherever it runs:
...
| `--format <name>`  | `markdown`, `json`, or `plain`            | [auto](#output-format) |
...
<Callout title="Output formats">
  In a terminal, the default `markdown` output is a best-effort render. For
  non-trivial docs prefer `--format json` (a structured payload) or `--format
...
| Flag        | Description                                               | Default                |
| ----------- | --------------------------------------------------------- | ---------------------- |
| `--format`  | `text` or `json` output                                   | [auto](#output-format) |
