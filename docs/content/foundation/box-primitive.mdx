...
to allowed values defined by the system can be very restrictive, you can use the `classNames` prop and generate custom
styles with the <Link to="../../guides/usestyles">`useStyles` hook</Link>.

```tsx live
<Box classNames={useStyles({ css: { fontSize: '11pt' } })}>small text</Box>
```
