...
With the factory in place, components stay out of two responsibilities. They never assemble a query string or know an endpoint's shape, and they never spell out a raw key array like `['venues', filters]`. A component passes a domain value such as a venue id, the hook builds the request, and the factory builds the key. Because every list key descends from `venueKeys.lists()`, a mutation re-syncs all of them with a single call rather than guessing which inline keys were created elsewhere.
