...
1. the XLoader animation appears within an overlay (created via a [dialog](https://www.marigold-ui.io/components/overlay/dialog)) over the container of the loading content.
   a. If the context requires additional information, add a label. Use labels sparingly, like at the start or end of important tasks or at particularly confusing junctions.
2. If the button used to start the process is visible and located outside the container of the loading content, it follows the process for inline loading states as well.
3. The mouse cursor style changes to progress.
...
Feedback messages that appear after the process has produced a result should clearly communicate what’s happened and, if appropriate, what actions the user can take next. Follow the [content guidelines for feedback messages](https://www.marigold-ui.io/patterns/feedback-messages).
