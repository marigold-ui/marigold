...
Badges are too heavy for actions that render as inline text links. Use the "master" or "admin" variant on the `<Link>` component instead. The variant prepends a small glyph in the matching access color, a lock for `master` and a key for `admin`. The glyph also carries the access level as alternative text, so screen reader users can tell a restricted action from a regular one.

<ComponentDemo name="admin-master-mark-inline-actions" />

### Menus

The same variants are available on `<ActionMenu.Item>` and render the same glyph. In addition, group restricted items into dedicated sections such as "Master actions" or "Admin actions". The section title makes the access boundary visible at a glance and announces the access level to screen reader users. Restricted items should not carry an additional icon, since the glyph already occupies the item's icon slot.

<ComponentDemo name="admin-master-mark-menu" />

#### Combining access marks with other variants

The `variant` prop is a single axis. A menu item or link can use either an access variant or another variant like `destructive`, but never both at once. This is intentional. A red destructive treatment stacked on the access glyph would put two competing signals on one small element.
