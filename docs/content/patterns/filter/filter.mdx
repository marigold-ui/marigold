...
```js
// TODO: need some visual break here to have not too much text. maybe do/donts?
```
...
When constructing labels for applied filters, use a clear structure so users can immediately understand what the label means. The recommended method is to combine three parts: the identifier (what is being filtered, such as category, property, or type), the relative or absolute operator (how the filter applies, for example is, is above, is between, or a direct match), and the value (the specific number, threshold, or date).
...
Whether using absolute or relative values, apply the same format throughout the interface. Consistency helps users quickly scan and understand their filters, which is especially important in complex filtering scenarios.

```js
// TODO: Demo showing some filter examples
```
