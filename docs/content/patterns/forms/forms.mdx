...
Forms with multiple sections should maintain spacing between sections using `<Stack space='section'>`. Each section should include a `<Headline>` as part of its field stack to provide clear context.

<ComponentDemo file="./form-spacing-section.demo.tsx" mode="preview" />
...
Proper spacing between fields creates a clear visual hierarchy and improves form readability. Use `regular` as the baseline vertical spacing between individual fields within a section, and `related` for horizontal spacing between paired fields on the same line (e.g. Postal Code + City).

Use `tight` when placing a description directly under a section headline to keep the headline and its supporting text closely associated.

<ComponentDemo file="./form-spacing-field.demo.tsx" mode="preview" />

When grouping related fields, such as address components or date and time fields, you can create visual associations that make the form easier to understand. Fields within a group use `regular` spacing, while `group` spacing goes _between_ sets of grouped fields to create a visual break. This pattern is especially helpful for sections with many fields, where you need to separate logical clusters (e.g. an address block from a contact block) without introducing a full section break.
...
<Table aria-label="spacing tokens">
  <Table.Header>
    ...
    <Table.Column>CSS Variable</Table.Column>
  ...
  <Table.Body>
    <Table.Row key={0}>
      ...
      <Table.Cell>Max-width for the form container.</Table.Cell>
      <Table.Cell>`<div>`, `<Form>`</Table.Cell>
    </Table.Row>
    <Table.Row key={1}>
      <Table.Cell>`section`</Table.Cell>
      <Table.Cell>`--spacing-section`</Table.Cell>
      <Table.Cell>Space between different form sections.</Table.Cell>
      <Table.Cell>`<Stack>`</Table.Cell>
    </Table.Row>
    <Table.Row key={2}>
      <Table.Cell>`regular`</Table.Cell>
      <Table.Cell>`--spacing-regular`</Table.Cell>
      <Table.Cell>Baseline vertical space between form fields within a section.</Table.Cell>
      <Table.Cell>`<Stack>`</Table.Cell>
    </Table.Row>
    <Table.Row key={3}>
      <Table.Cell>`group`</Table.Cell>
      <Table.Cell>`--spacing-group`</Table.Cell>
      <Table.Cell>Space between groups of related fields within a section.</Table.Cell>
      <Table.Cell>`<Stack>`, `<Inset>`</Table.Cell>
    </Table.Row>
    <Table.Row key={4}>
      <Table.Cell>`related`</Table.Cell>
      <Table.Cell>`--spacing-related`</Table.Cell>
      <Table.Cell>Space between fields that form a unit (e.g. postal code + city, start date + end date).</Table.Cell>
      <Table.Cell>`<Inline>`</Table.Cell>
    </Table.Row>
    <Table.Row key={5}>
      <Table.Cell>`tight`</Table.Cell>
      <Table.Cell>`--spacing-tight`</Table.Cell>
      <Table.Cell>Space between a headline and its description text.</Table.Cell>
      <Table.Cell>`<Stack>`</Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
