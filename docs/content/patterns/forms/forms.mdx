...
Other components for form structure:
...
- **Feedback components**: You can use feedback components like `<Tooltip>`, `<SectionMessage>`, and `<Toast>` to provide additional information or context.
...
- **Multi-step processes**: When breaking long forms into smaller steps, sections serve as natural breakpoints and represent manageable progress toward completion.
...
For a complete reference of all available spacing options, see our [spacing tokens](#spacing-tokens) section.
...
<Table aria-label="spacing tokens">
  ...
  <Table.Body>
    ...
    <Table.Row key={3}>
      ...
      <Table.Cell>Used for grouping related fields together, such as address components or date and time fields.</Table.Cell>
