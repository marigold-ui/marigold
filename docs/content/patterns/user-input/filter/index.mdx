...
### Collapsing Applied Filters

With several filters active, stacked rows of applied filter tags can push the data set out of the viewport and make the applied filters area harder to scan. Use the [`collapseAt`](/components/collection/tag#props) prop on `Tag.Group` to show only the most recent filters and collapse the rest behind a "Show more" toggle, keeping the row height predictable. The example below collapses after 3 filters.
