...
Use a [Badge](/components/content/badge) inside the option's label to display the count. Always place the count inside the label, regardless of the control used. This keeps the placement consistent across filter types and ensures assistive technology announces the count together with the option.
