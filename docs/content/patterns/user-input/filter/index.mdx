...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| **Single**   | One option from a set, such as a category or status. Use a radio group when a few options are all worth showing, or a [Select](/components/form/select) when space is tight or the list is long.                                                     | [Radio](/components/form/radio)              |
...
| **Contains** | A keyword or partial match typed by the user. Use [SearchField](/components/form/search-field) in the toolbar, or a [TextField](/components/form/textfield) for a field inside the panel.                                                            | [SearchField](/components/form/search-field) |
...
| **Boolean**  | A single on/off condition, such as "In stock only". A switch reads as an instant on/off and suits a toolbar quick filter. Inside the panel, a single checkbox does the same job.                                                                     | [Switch](/components/form/switch)            |
...
- **Toolbar controls apply instantly.** The search field, along with any filter promoted into the toolbar as a [quick filter](#quick-filters), takes effect the moment the user changes it. The cost is a query on every interaction and results that reflow on each change.
...
Because the model is tied to location, the two stay legible side by side. A bare toolbar control signals that it takes effect now, while the panel and its Apply button signal that the user composes first and commits once. Keep that boundary clean. A batched control sitting loose in the toolbar, or an instant control buried inside the panel, leaves users unable to tell whether their change has taken effect.
...
The techniques above help within a single filter, but the set of filters can grow large too. Two moves keep it manageable as it grows: pull the few most-used filters out into the toolbar so the common case never requires opening the panel, then organize the rest so the panel stays easy to scan.

### Quick Filters

When usage is concentrated on a few filters, hiding them in the panel adds an unnecessary step to the most common task. Establish a two-tier hierarchy: surface the few high-value filters as quick filters directly in the toolbar, and keep the complete set behind an "All filters" button that opens the panel. The panel stays canonical: a quick filter is a shortcut to a filter that still lives in the panel and shares its state, so anyone who opens "All filters" first still finds it.
...
For the controls themselves, prefer something compact such as a [Select](/components/form/select). Quick filters apply instantly, so see [Applying Filters](#applying) for how that model works alongside the panel's batched commit.
...
Relative date-range presets are composed in userland today.
