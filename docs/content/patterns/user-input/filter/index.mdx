...
For the controls themselves, use something compact and field shaped, such as a [Select](/components/form/select), so the bar reads as one family of inputs. A [SegmentedControl](/components/form/segmented-control) is not a quick filter but a scope switch: it recasts the whole view along one dominant dimension, such as All, Active, and Archived, and sits at the start of the bar, apart from the field cluster. Quick filters apply instantly, so see [Applying Filters](#applying) for how that model works alongside the panel's batched commit.
