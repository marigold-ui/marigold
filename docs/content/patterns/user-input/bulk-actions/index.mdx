...
The pattern is written around a `<Table>` because select-all needs a header checkbox, and a table is the collection that has a header to put one in. A [`<ListView>`](/components/collection/listview) carries everything else: set `selectionMode="multiple"` and render the `<ActionBar>` as a sibling, passing `selectedItemCount` and `onClearSelection` yourself. Reach for it when users pick a handful of rows by hand, and for a `<Table>` when selecting every visible record is part of the job.
...
<TeaserList
  items={[
    ...
    },
    {
      ...
      href: '/patterns/feedback/destructive-actions',
      caption:
        'Whether a destructive action needs a confirmation dialog or can offer an undo, and how to build either.',
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
