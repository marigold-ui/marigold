...
While the bar is visible, the selection is the object the user works on. Avoid competing per-row actions during that time: a row-level delete next to a bulk delete makes it unclear which scope a press affects. Bulk and per-row actions can still share a table — one-off operations belong in a row menu (see [Table Records](/patterns/user-input/table-record-management)), repetitive ones in the bar — as long as they take turns: while rows are selected, disable the row actions so the bar holds the only pressable scope.
