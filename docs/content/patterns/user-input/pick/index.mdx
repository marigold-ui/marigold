...
A minimum is the fewest the user may pick. Keep the commit button disabled until they reach it, so a set that is too small can never be committed by accident. Most picks need at least one record, so the [example](/examples/pick) disables its button while nothing is chosen. If a pick needs more than one, do the same, and let the helper text say how many more are still needed.
