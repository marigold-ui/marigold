...
Name the outcome with the host task's own verb and carry the staged count, so the button says what pressing it does, the same reason a results button reads "Show 82 results" rather than "Show". The verb comes from the task, not the pick: "Add 3 venues" when the set joins a collection, "Assign 3 users" when it grants access, or "Save 3 venues" when the commit writes the set into a record the user keeps and edits, as the [example](/examples/pick) does.

When nothing is staged, show the bare verb ("Add venues") disabled rather than a clickable "Add 0 venues", and compose the label by hand from the count so the wording and the number never disagree. The [`<ActionBar>`](/components/collection/actionbar) is built for a table's floating bulk actions, not a dialog's commit footer, so it is not the tool for this button.

<GuidelineTiles>
  <Do>
    <DoDescription>
      Name the outcome and carry the count with the host task's verb, so the
      label reads "Add 3 venues", "Assign 3 users", or "Save 3 venues" for a set
