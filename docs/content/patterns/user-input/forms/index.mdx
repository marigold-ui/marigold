...
This pattern works well when each section can be saved independently. Place a `<Form>` inside `<Panel.Content>` and connect the submit button in `<Panel.Footer>` using the HTML `form` attribute.
...
A fictional event registration form that puts all of these guidelines together: sections, field types, and actions. Uses a single submit button for the entire form.
...
A full-page settings form with multiple independent sections, each with its own save action. Uses Panels to structure sections, tabs for navigation, and a danger zone for destructive actions.
