...
By default, errors appear after the user leaves the field or submits the form. Real-time validation makes sense in narrow situations like password rules, where users benefit from seeing requirements light up as they type. [Control the field](https://react.dev/learn/sharing-state-between-components#controlled-and-uncontrolled-components) and set `error` and `errorMessages` yourself.

<ComponentDemo file="./validation-realtime.demo.tsx" />

### Server errors

Client-side validation is for fast feedback; server-side validation is what actually protects data. Pass server errors back through the `validationErrors` prop on `<Form>`, keyed by field `name`. The errors appear immediately and clear themselves when the user edits the field.
...
The simplest way to read a form is the browser's [FormData](https://developer.mozilla.org/en-US/docs/Web/API/FormData) API. Wrap your fields in `<Form>` and React 19's action prop will hand you the `FormData` instance directly.
...
For larger forms with many fields and rules, [react-hook-form](https://react-hook-form.com/) cuts the boilerplate and minimizes re-renders. Wrap each Marigold field with `<Controller>` so the library can track its value and validation state.
