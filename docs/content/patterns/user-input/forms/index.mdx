...
Forms need to capture user input, react to it, and hand it off when the user submits. Marigold's form components work with both uncontrolled and controlled patterns, and integrate cleanly with native `FormData`, React 19 actions, and libraries like react-hook-form.
...
Marigold's form components accept `error` (boolean) and `errorMessage` (string) props. Combine them with controlled state to display field-level errors from any source: client validation, server response, or library output.

<ComponentDemo file="./error-handling.demo.tsx" background="page" />

### Using react-hook-form

For larger forms with many fields and rules, [react-hook-form](https://react-hook-form.com/) cuts the boilerplate and minimizes re-renders. Wrap each Marigold field with `<Controller>` so the library can bridge the field and its own state management.

A few patterns worth knowing when wiring it up:

- **Set `defaultValue` instead of state.** Each Controller takes a `defaultValue`, so fields stay uncontrolled and never go through an `undefined`/`null` state.
- **Declare rules on the Controller.** Pass `rules={{ required: true }}` (and friends) to trigger validation. The Marigold field already accepts `error` and `errorMessage`, so wire those from the Controller's render props.
- **Gate submission with `formState.isValid`.** The library exposes a single boolean for the whole form, which is handy for disabling the submit button until every field is valid.

<ComponentDemo file="./building-forms-hook-form.demo.tsx" background="page" />
