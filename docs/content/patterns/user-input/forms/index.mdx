...
Most forms fall into one of three patterns. Pick the one that matches your use case:

- **Task forms**: user is completing a task and submits once.
- **Settings forms with save on submit**: config edited over time, batched per section, explicit commit.
- **Settings forms with auto-save**: toggles and preferences that persist instantly without a save action.

The demos below show each end-to-end so you can see how the pieces fit together.
...
### Settings forms with save on submit

Use for configuration that's edited over time when changes should be batched and explicitly committed: organization defaults, billing details, anything with cross-field validation or destructive consequences. Users can review their edits before saving and back out by not submitting.
...
- Avoid for fields that need cross-field validation, a confirmation step, or any change that's costly to reverse. Use [save on submit](#settings-forms-with-save-on-submit) instead.
