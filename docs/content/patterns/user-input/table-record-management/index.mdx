...
The same drawer handles both jobs. An "Add" button opens it with an empty form for creation. An "Edit" button on each row opens it pre-filled with that record for editing. The title and the submit behavior follow the mode, but the form layout is identical, so users learn one screen instead of two.
