...
Inline links are links that are embedded within text content, allowing users to navigate to related information without leaving the current context. For this purpose, you should use the `secondary` variant of the `<Link>` component. The `secondary` variant is designed to be less visually prominent than the primary link, making it suitable for inline usage.
