...
The hierarchy in the `<Headline>` component is essential for both usability and accessibility. Visually, it guides users through the content by clearly indicating the structure of a page—larger, more prominent headlines signal more important sections, while smaller ones denote subsections. You should use the `level` prop (or `size`) to define the headline level, which corresponds to HTML headline tags (`h1`, `h2`, etc.).
