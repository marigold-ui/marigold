...
- **[ErrorState](/components/content/error-state)**: When a region's content is gone because an error boundary caught a failure, replace it with an `ErrorState`. Use `SectionMessage` only to annotate content that is still visible. See the [error states pattern](/patterns/feedback/error-states).
