...
A `<SectionMessage>` is a container with a severity icon, a title, the message content, and an optional dismiss button.

<SectionMessageAnatomy />

- **Container**: Wraps the message and applies the variant styling.
- **Icon**: Variant icon that reinforces the severity of the message.
- **Title**: Short headline that summarizes the message.
- **Content**: Descriptive body text below the title.
- **Close button**: Optional dismiss control when `closeButton` is set.
...
Set the `announce` prop and `<SectionMessage>` will fire the announcement for you. You don't need to wrap the message in your own live region.
...
<GuidelineTiles>
  <Do>
    <DoDescription>
      Use `announce` for messages that appear in response to a user action, so
      assistive technology users hear about the change.
    </DoDescription>
  </Do>
  <Dont>
    <DontDescription>
      Don't set `announce` on a message rendered on initial page load. It will
      fire on hydration and interrupt the user during their first scan of the
      page.
    </DontDescription>
  </Dont>
</GuidelineTiles>
