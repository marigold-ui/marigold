...
A kebab menu is the "⋮" overflow trigger that gathers the actions for one specific item, such as a table row, a card, or a list entry. Use the [`<ActionMenu>`](/components/overlay/action-menu) companion for it. It shows a compact icon-only ghost trigger, switches to a `<Tray>` on small screens, and takes the same `<Menu.Item>` API as `<Menu>`.
