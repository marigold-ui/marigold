...
For quick, context-specific actions on an item or row, see [`<ActionMenu>`](/components/overlay/action-menu). It uses an `<ActionButton>` as its trigger, switches to a `<Tray>` on small screens, and follows the same `<Menu.Item>` API.
