...
The `<Toast>` Component should be in the root of your application, so it can be used anywhere in your app. You can use the `addToast` function to add a Toast to the queue. The Toast will appear at the bottom right of the screen by default. with the position prop you can change the position of all Toasts.
...
`addToast` does return the key of the Toast, with this the Toast can be dismissed programmatically, by saving the key of the Toast and then using `removeToast`. All Toasts can be dismissed with `clearToasts`.
