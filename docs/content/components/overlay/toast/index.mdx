...
This is what an "Undo" toast defers its work with. Put the commit in `onClose` rather than in a `setTimeout` of the same length: React Aria pauses a Toast's timer while the toast region is hovered or focused, so a separate timer drifts out of sync and commits while the Undo button is still on screen.

<Callout type="warning" title="Only for toasts that dismiss themselves">
...
See [Destructive Actions](/patterns/feedback/destructive-actions#offering-undo) for the whole undo pattern, including how to guard the commit so pressing Undo cancels it.
...
<AutoTypeTable path="Toast/ToastQueue" name="ToastOptions" />
