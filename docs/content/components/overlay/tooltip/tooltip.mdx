...
The `<Tooltip>` components allow to provide additional information to that user. For that the `<Tooltip>` is divided into a `<Tooltip.Trigger>` and the `<Tooltip>` itself. You need to wrap the trigger around the `<Tooltip>` and a simple button, which is necessary to trigger openning and closing the tooltip.
...
<Dont>
  <Dont.Description>
    Don't user more than 2-4 tooltips per page.
  </Dont.Description>
</Dont>
