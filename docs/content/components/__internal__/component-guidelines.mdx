...
#### Disabling Chromatic snapshots

Every story is snapshotted by Chromatic for visual regression testing by default.
You can opt a story out with `parameters.chromatic.disableSnapshot: true`, but this
permanently removes that story from visual coverage, so only do it when the snapshot
adds no signal.

**Disable the snapshot when:**

- The story is a **visual duplicate** of another story that is still snapshotted
  (e.g. a base/showcase frame already captures the same rendering).
- The story shows a **closed/default trigger state** whose open/expanded visual is
...
- [Color design tokens](../foundations/token-overview) need to have a semantic name that reflects their use case. (e.g., `bg-disabled-surface` for disabled backgrounds)
...
- Style props are using tokens from TailwindCSS by default.
