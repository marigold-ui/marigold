...
The column named by `treeColumn` renders the expand control and the indentation automatically. You don't place a chevron yourself, and rows without children reserve the same width so values stay aligned down the column. Group rows are emphasised for you, so there is no need to style them.
