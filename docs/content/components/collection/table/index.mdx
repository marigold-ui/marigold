...
Not supported yet, and worth knowing before you design around it: sorting that reorders levels automatically (you sort your own data, so a grouped table has to sort each level), and there is no built-in "expand all" control — set `expandedKeys` to every group's key yourself.

Drag and drop is partly there. Reordering works, and the insertion line is indented to the level the row would land at, so a drop inside an expanded group reads as landing in that group. Moving a row _between_ levels is up to your drop handlers, and the keyboard path only offers root-level positions, so don't ship cross-level reordering as keyboard-accessible yet.
