...
Moving between two fields stays a grid movement rather than a single <kbd>Tab</kbd> press, so the keystrokes add up faster than the table looks. That cost is what bounds the approach. See [Filling in many rows at once](/patterns/user-input/table-record-management#filling-in-many-rows-at-once) for the limits it sets on columns and rows, which cells to keep read-only, and how the grid is saved.
