...
When child rows come from data, render them with `<Table.ChildRows>` and let your render function recurse. It renders nothing when `items` is empty or missing, so leaf rows need no special case.
...
<AutoTypeTable path="Table/TableChildRows" name="TableChildRowsProps" />
