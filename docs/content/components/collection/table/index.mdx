...
The row header also gives the row its name for type to select, where typing a few
characters moves focus to the first matching row. That name is read from the row
header cell's content, so a cell holding a plain string or a number needs nothing
extra.

Content that is not plain text cannot be read, so such a cell states its own
`textValue`. In development, a row header cell that needs one and does not have it
logs a warning.
...
A `textValue` on `Table.Row` names the whole row and overrides anything its cells
derive. Prefer the cell, which sits next to the content it describes and cannot
drift when columns are reordered.

`Table.EditableCell` follows the same rule, reading its name from the display
content it shows when it is not being edited.
