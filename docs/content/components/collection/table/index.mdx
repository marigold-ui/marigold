...
When the nested rows come from data, render them with `<Table.ExpandedRows>` and let your render function recurse. It renders nothing when `items` is empty or missing, so leaf rows need no special case. Always render it — the Table decides whether the rows are visible, so don't wrap it in a check of the expansion state.
...
Mark such a row `expandable` to break that cycle. It gets a control and announces itself as collapsed before its rows exist, and you load them in `onExpandedChange`. You don't need it for rows you pass in up front, whether nested statically or through `<Table.ExpandedRows>`.
...
<AutoTypeTable path="Table/TableExpandedRows" name="TableExpandedRowsProps" />
