...
Set `loading` while the table's data is being fetched. The table is marked as busy and screen readers announce that it is loading. Nothing is announced when loading finishes, since the rows appearing are the signal.
