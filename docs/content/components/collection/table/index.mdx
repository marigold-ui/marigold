...
- **Select-all covers rendered rows only.** Because collapsed children aren't in the DOM, the header checkbox selects the visible rows and skips the children of collapsed groups — so "select all" does not mean "every record".

If you need cascading selection with a partial state, model it in your own data and derive `selectedKeys` from it, or use a flat table with an explicit grouping column instead.
