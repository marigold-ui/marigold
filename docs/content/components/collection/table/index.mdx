...
The column named by `treeColumn` renders the expand control automatically — you don't place a chevron yourself. The control sits in a leading gutter that every row reserves, so a group row, its children and an ungrouped row all start their value at the same x and the column stays one scannable column. That matters most for the identifier people search on: staggering a clearing number by nesting level is what makes it hard to find by eye. Containment is carried by the group row instead, which is filled and emphasised for you, so there is nothing to style.
...
Not supported yet, and worth knowing before you design around it: drag and drop across nesting levels, and sorting that reorders levels automatically (you sort your own data, so a grouped table has to sort each level). There is no built-in "expand all" control either — set `expandedKeys` to every group's key yourself.
