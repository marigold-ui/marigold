...
Show only the most important 1 to 3 actions per row. These should be the actions users are most likely to need or expect to find without having to open a menu, such as "Edit" or "View." If you need to support more than three actions, prioritize them by frequency and importance: display the most critical ones directly, and group the rest into a contextual menu using an `<ActionMenu>` (see [ActionMenu](/components/overlay/action-menu)). This keeps the row clean and scannable while still offering access to all necessary actions.
...
<Callout title="Children that load on demand" type="warning">
...
<GuidelineTiles>
  ...
  <Dont>
    <DontDescription>
      ...
      [Accordion](/components/navigation/accordion) when the expanded content
      doesn't share the table's columns.
    </DontDescription>
  </Dont>
</GuidelineTiles>
...
Use `loading` whenever the rows are about to change because data is on its way: the first fetch when the page opens, and every refetch after a user sorts, filters, searches or pages through the table. Sighted users can see a spinner or stale rows being replaced, while screen reader users would otherwise hear nothing and may start acting on rows that are about to disappear. With `loading` set, they hear that the table is loading. Nothing is announced when it finishes, because the new rows are the signal.

Keep it tied to the fetch that feeds this table. A save or an export that leaves the rows as they are is not a loading state, so give it its own feedback, such as a [Toast](/components/overlay/toast) once it is done.

```tsx
<Table aria-label="Orders" loading={isLoading}>
  …
</Table>
```

Use `loading` instead of passing `aria-busy` to the table. React Aria drops that attribute, so it never reaches the page.
...
The row header also names the row for type to select, where typing a few
characters moves focus to the first matching row. A cell holding plain text or a
number needs nothing extra. For any other content, set `textValue`.

```tsx
<Table.Cell textValue={user.name}>
  <Stack space="0.5">
    <Text weight="medium">{user.name}</Text>
    <Text size="xs">{user.handle}</Text>
  </Stack>
</Table.Cell>
```

Type to select matches from the **start** of the value, so lead with what someone
would actually type. `"Jane Doe"` is findable by name, while `"4711 Jane Doe"` is
findable only by its number.

`Table.EditableCell` works the same way, using the content it shows when it is not
being edited.

### Accessibility

A table with [grouped rows](#grouped-rows) reports itself as a `treegrid` instead of a `grid`, which is what lets screen readers announce how deep a row sits and whether it is expanded. Tests that query `getByRole('grid')` have to move to `getByRole('treegrid')`.

Group rows expand with <kbd>→</kbd> and collapse with <kbd>←</kbd>. The expand control stays out of the tab order, so <kbd>Tab</kbd> keeps moving between the buttons and links inside the rows.
...
<ul>
  ...
  <li>
    [Cards](/components/collection/card): Helpful if the data needs to be
    displayed with more visual hierarchy. Keep in mind that you can't compare
    data in this way not as good as tables, because the eyes have to move much
    more.
  </li>
  <li>
...
Is there still not the right alternative for you please [get in touch](/getting-started/get-in-touch) with us!
...
<TeaserList
  items={[
    ...
    {
      title: 'useAsyncList',
      href: '/components/hooks-and-utils/useAsyncList',
