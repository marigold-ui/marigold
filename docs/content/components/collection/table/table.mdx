...
Use the `<Button>` component with the secondary (default) variant for row actions. If the action is destructive, apply the destructive variant instead. Users should always know where to find row actions, so always place them at the end of the row. This improves scannability and helps clarify which parts of the row are interactive.
