...
Use the `muted` variant for table to reduce visual noise and improve clarity, especially in tables with extensive content. This helps users focus on the data while maintaining a clean and organized appearance.

#### Grid

The `grid` variant is useful if you want clearer separation between columns, especially for tables with many columns or dense data. The grid variant adds vertical borders between columns, making it easier to scan and compare values across rows.
...
In this example an [`<ActionMenu>`](../overlay/menu) is used to make interaction possible. The table should only be used with one action cell at the end of the columns.
