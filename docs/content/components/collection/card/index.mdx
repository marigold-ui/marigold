...
Reach for [`<Table>`](../table) when items share the same fields and the user's task is comparison, sorting, filtering, or [bulk action](/patterns/user-input/bulk-actions). Reach for `<Card>` when each item is recognized by its own visuals and links to a detail view, not by its position in a row.
