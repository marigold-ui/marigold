...
A link that opens a new window or tab marks itself. It adds a small external-link icon after the label plus a hidden "opens in a new window" warning, so screen reader users are told about the change too. Targets that stay in the current window (`_self`, `_top`, `_parent`) are left alone, and so are links that cannot navigate at all, meaning `disabled` ones and ones with no `href`.
