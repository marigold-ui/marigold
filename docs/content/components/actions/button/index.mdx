...
A few containers deliberately do the opposite. [`<EmptyState>`](/components/content/empty-state), [`<ErrorState>`](/components/content/error-state), [`<SectionMessage>`](/components/content/section-message), and [`<ContextualHelp>`](/components/overlay/contextual-help) reset the cascade to a clean baseline, so their action never inherits the ghost or small treatment of a container further up. A `<Button>` in one of them keeps its own default `secondary`, and you pick the variant yourself.
