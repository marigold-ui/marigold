...
`<Button>` is the right call for a page-level commitment — submit, save, confirm, continue. The same `<Button>` also serves as a low-emphasis **surface action**: the chrome that belongs to a row, a toolbar, or a container header rather than to the page. Reach for `variant="ghost"` so the button recedes into its surroundings instead of competing with the page's primary action.
