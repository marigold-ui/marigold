...
### Button or ActionButton

For low-emphasis actions that belong to a row, a toolbar, or a container's chrome, reach for [`<ActionButton>`](/components/actions/action-button) instead. `<ActionButton>` picks up its size and visual weight from the surrounding component, so the action stays in the background of its surface rather than competing with `<Button>` for attention. See [`<Button>` vs. `<ActionButton>`](/components/actions/action-button#button-vs-actionbutton) for the full decision table.
