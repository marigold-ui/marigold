...
`<Button>` is the right call for the page-level commitment: submit, save, confirm, continue. Marigold ships a small family of related components for the cases where `<Button>` is the wrong fit, like low-emphasis actions inside rows and surfaces, bulk-selection bars, overflow menus, and navigation. Pick the component by the role of the action, not by the variant you have in mind.
...
| Per-row action in a list, table, or `<SelectList>` option     | A ghost `<Button>` inside a [`<ButtonGroup>`](/components/actions/button-group). For actions that navigate (e.g. an "Edit" link to a detail page), use [`<LinkButton>`](/components/actions/link-button), which picks up the same cascade. |
| Chrome on a `<Panel>`, `<Dialog>`, or `<Drawer>` header       | A ghost `<Button>` (or a [`<ButtonGroup>`](/components/actions/button-group) of them) — it adapts to the container automatically                                                                                                           |
...
The same `<Button>` that carries a page-level commitment also serves as a low-emphasis **surface action** — the chrome that belongs to a row, a toolbar, or a container header rather than to the page. Reach for `variant="ghost"` so the button recedes into its surroundings instead of competing with the page's primary action.
...
`<Button>` is **slot-aware**: dropped into a container that publishes a `ButtonContext` — a [`<ButtonGroup>`](/components/actions/button-group), a [`<Panel.Header>`](/components/layout/panel), or a [`<SelectList.Option>`](/components/form/select-list) — it inherits its `size`, `variant`, and placement from that container, so the call site stays free of repetitive props. A local prop always wins over the inherited value, and `slot={null}` opts a button out of the cascade entirely.
...
- [ButtonGroup](/components/actions/button-group): A toolbar wrapper around a related cluster of buttons; cascades a shared `variant`/`size`.
- [ActionBar](/components/actions/actionbar): The bar around bulk-selection actions; uses `<ActionBar.Button>` internally.
