...
<SectionMessage variant="warning">
  <SectionMessage.Title>Removed className Support</SectionMessage.Title>
  <SectionMessage.Content>
    Custom styling via `className` is no longer supported. Instead replace
    overrides with the component's built-in `variant` or `size` props. If
...
To visually imitate a button element that navigates (e.g., a "Cancel" action that redirects users to the previous page), use the [`<Link>`](/components/actions/link) component with the corresponding `variant`, usually `secondary`. This way, the action is clear and prominent while keeping its navigation function.
