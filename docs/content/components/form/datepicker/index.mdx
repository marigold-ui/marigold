...
Use the `presets` prop to offer common dates as a one-click list beside the calendar in the popover. Built-in keys ship with localized labels and correct date math, and custom presets take a `label` plus a date value or a resolver function. Presets that fall outside `minValue`/`maxValue` or land on an unavailable date are disabled.
