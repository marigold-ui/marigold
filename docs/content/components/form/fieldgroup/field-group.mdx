...
<Message messageTitle="For your information !" variant="info">
  The `FieldGroup` component is created to handle complex layout in `theme-core`
  specifically, by using the `labelWidth` property that sets the width of all
  fields labels inside the `FieldGroup` .
</Message>
