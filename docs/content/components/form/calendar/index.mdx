...
When users need to select dates far in the past or future (such as a birth date or historical event), a calendar grid forces tedious month-by-month navigation. In those cases, a `<DateField>` or `<DatePicker>` is more efficient.
...
<GuidelineTiles>
  ...
  <Dont>
    <DontDescription>
      Don't show all dates as selectable when some are invalid. This leads to
      errors and forces users to guess which dates are allowed.
    </DontDescription>
  </Dont>
</GuidelineTiles>
