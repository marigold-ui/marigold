...
The `<Radio>` should never be used alone because radio buttons are designed for selecting one option from multiple mutually exclusive choices. For that the `<Radio.Group>` should be wrapped around the `<Radio>`. If you want one option you could use the [`<Checkbox>`](/components/form/checkbox) instead.
...
<GuidelineTiles>
  <Do>
    ...
    <DoDescription>Use select component with long options</DoDescription>
  </Do>
  <Dont>
    <DontFigure>
      <ImageZoom
    ...
    <DontDescription>Don't use radio with long options.</DontDescription>
  </Dont>
</GuidelineTiles>
...
To mark a single option with a `<Badge>`, such as an access level or a "New" flag, pass it to `labelAdornment` rather than putting it inside the radio's children. The slot holds it to the height of the label's line, so the dot stays centred on the text instead of drifting above it, and a label that wraps still keeps the dot on its first line. Use `<Badge size="inline">`, which is drawn to fit a line of text.

<ComponentDemo name="radio-label-adornment" />
...
<StorybookHintMessage component={'Radio'} />

### Radio

<AutoTypeTable path="Radio" name="RadioProps" />

### Radio.Group

<AutoTypeTable path="Radio/RadioGroup" name="RadioGroupProps" />
...
<ul>
  ...
  <li>
    [SelectList](/components/form/selectlist): When each option needs a
    description line to help the user decide, or an overflow action, use a
    `<SelectList>` instead.
  </li>
</ul>
