...
To mark a single option with a `<Badge>` — an access level, a "New" flag — pass it to `labelAdornment` rather than putting it inside the radio's children. The slot holds it to the height of the label's line, so the dot stays centred on the text instead of drifting above it, and a label that wraps still keeps the dot on its first line. Use `<Badge size="inline">`, which is drawn to fit a line of text.
