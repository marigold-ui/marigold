...
To mark a single option with a `<Badge>`, such as an access level or a "New" flag, pass it to `badge` rather than putting it inside the radio's children. The slot holds it to the height of the label's line and sizes it to fit, so the dot stays centred on the text instead of drifting above it, and a label that wraps still keeps the dot on its first line.
