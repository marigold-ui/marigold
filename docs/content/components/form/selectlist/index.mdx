...
A horizontally arranged list automatically flips to a vertical stack once its **wrapping container** is narrower than `28rem` (≈448px). The flip is driven by a CSS container query on the list's own wrapper, so it reacts to the parent it is rendered into a sidebar, a drawer, a narrow card not just the viewport width. Keyboard navigation continues to work in both axes after the flip.
