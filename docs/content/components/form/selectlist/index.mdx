...
### `<SelectList>` vs. `<ListView>`

A [`<ListView>`](/components/collection/listview) renders the same stack of rich rows and can select them too, so the two are easy to confuse. One question separates them:

> **Does the selection need to submit with the form?**
>
> - **Yes**, it is a field value that submits and validates: `<SelectList>`.
> - **No**, you read it and commit it yourself: [`<ListView>`](/components/collection/listview).

The reason behind the test is what each component is. A `<SelectList>` is a form field. It carries a label, validation, and a hidden input, so its selection is the field's value and travels with the rest of the form. A `<ListView>` is a collection view. Its selection is view state the consumer owns and commits, and rows may carry their own focusable controls.

The case worth catching is a pick that never submits: records staged in a dialog and applied with its confirm button. A `<SelectList>` does work there, and it brings a label, a required marker, and validation wiring that the flow never asked for. Use a `<ListView>` for those, and keep `<SelectList>` for the field.
...
| When                                                         | Use                                                                                                              |
| ------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| Short list of plain text choices (3 to 5 items)              | [`<Radio.Group>`](/components/form/radio) (single) or [`<Checkbox.Group>`](/components/form/checkbox) (multiple) |
| Each option needs decision-relevant context (up to 10 items) | `<SelectList>`                                                                                                   |
| Space is limited, with or without rich rows (5 to 15 items)  | [`<Select>`](/components/form/select) (single or multiple)                                                       |
...
<GuidelineTiles>
  <Do>
    <DoDescription>
      Keep it to one action per row, and group multiples inside a single action
      menu rather than placing icons side by side.
    </DoDescription>
  </Do>
  <Dont>
    <DontDescription>
      Don't pair management actions like Edit or Remove with form selection.
      Mutating the underlying record mid-form breaks the flow with confirmation
      dialogs or network calls.
    </DontDescription>
  </Dont>
</GuidelineTiles>
...
<GuidelineTiles>
  <Do>
    <DoDescription>
      Use horizontal for short, peer-level choices that fit on one line, like a
      shipping speed or a sort order.
    </DoDescription>
  </Do>
  <Dont>
    <DontDescription>
      Don't use horizontal for long lists or rich rows that wouldn't read well
      stacked. Pick the orientation the content fits.
    </DontDescription>
  </Dont>
</GuidelineTiles>
...
- [Combobox](/components/form/combobox): Use for long lists where typing narrows results faster than scanning.
- [TagField](/components/form/tag-field): Use for multi-select with search, where users add and remove choices as tags.
- [ListView](/components/collection/listview): Use when the selection stays in your own state instead of submitting with a form.
