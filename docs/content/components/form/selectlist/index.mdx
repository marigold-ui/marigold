...
`<SelectList>` is fully keyboard accessible. Arrow keys move focus between options, <kbd>Space</kbd> toggles the focused option, and <kbd>Tab</kbd> reaches any action inside it. Arrow direction follows the layout. Use <kbd>Up</kbd>/<kbd>Down</kbd> when vertical, <kbd>Left</kbd>/<kbd>Right</kbd> when `orientation="horizontal"`. This two-tier model is what lets a per-row button or menu coexist with row selection without breaking keyboard navigation.
