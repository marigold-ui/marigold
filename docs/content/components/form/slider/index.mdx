...
- **Label:** Describes what the slider adjusts.
- **Output:** Displays the current value as text.
- **Track:** Shows the range of available values.
- **Thumb:** Marks the current value and can be dragged to change it.
- **Description:** Supplementary text below the slider.
...
Name both thumbs so users can tell the two ends of the range apart.

<ComponentDemo name="slider-range" />

### Labelling the thumbs

Give a thumb its own name whenever the slider's label does not already say what that thumb controls. Screen readers announce the thumb name ahead of the slider label, so a slider labelled "Fee split" whose thumb is named "Organiser share" tells the user exactly which side of the split they are about to move.

This matters most when one slider stands for two related quantities, or when a page holds several sliders adjusting the same kind of value. Set the names with the `thumbLabels` prop: one name for a single-thumb slider, one name per thumb for a range.

<ComponentDemo name="slider-thumb-labels" />
