...
When the loading state is activated, visible labels will disappear, as the purpose of the action should already be clear. To ensure accessibility, we provide the loading state with an `aria-label` and `aria-busy`.
