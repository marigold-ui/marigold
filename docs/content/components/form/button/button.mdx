...
The `<Button>` component includes a pending property that can be used to display loading behavior. This visually indicates the progress of system operations, such as downloading, uploading, or processing tasks.

This functionality is especially useful when performing actions like submitting a form or saving changes in your application that require time to complete a request. The button's label should clearly describe the action being performed, providing context for the current state of the process.
