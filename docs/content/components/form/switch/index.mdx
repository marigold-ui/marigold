...
On dedicated settings or preference pages, switches typically appear as a vertical list where the label is the primary scanning element. For this layout, use `variant="settings"` to place the label and description on the left and push the toggle to the far right. This is the layout people expect from system preferences and settings screens. Users read the label first to understand the setting, then look right to check or change its state. On mobile, it also places the toggle near the thumb for easier reach.

The default variant (toggle on the left) is better suited for compact, inline contexts like cards or list items where the switch is part of a larger element.
