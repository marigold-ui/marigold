...
To mark a switch with a `<Badge>`, such as an access level or a "New" flag, pass it to `labelAdornment` rather than building the decoration into `label` yourself. The slot holds it to the height of the label's line, so the track stays centred on the text instead of drifting above it, and a label that wraps still keeps the track on its first line. Use `<Badge size="inline">`, which is drawn to fit a line of text.

<ComponentDemo name="switch-label-adornment" />
...
<ComponentDemo name="switch-error" />
