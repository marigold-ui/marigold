...
<PropsTable
  props={[
    ...
    {
      property: 'onSubmit',
      type: '(id: string | number | null, value: string | null) => void',
      description: `Handler that is called when the SearchAutocomplete is submitted.
        
      A "id" will be passed if the submission is a selected item (e.g. a user clicks or presses enter on an option). If the input is a custom "value", "id" will be "null".
