...
<GuidelineTiles>
  <Do>
    ...
    <DoDescription>Use a around 5-15 options for the select.</DoDescription>
  </Do>
  <Dont>
    <DontFigure>
      <ImageZoom
...
Sometimes the trigger should look different from the option. For example, you might want to show an avatar plus a name in the trigger but include the role in the dropdown. Pass a `renderValue` callback to customize what the trigger displays. It receives the selected items and replaces the default trigger render whenever something is selected. The placeholder still shows when nothing is selected.

The returned content must not include focusable or interactive elements. The trigger is itself a button.

<ComponentDemo name="select-render-value" />
