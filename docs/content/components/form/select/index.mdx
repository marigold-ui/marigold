...
When the trigger should look different from the option (for example, an avatar plus a name in the trigger but avatar, name, and role in the dropdown), pass a `renderValue` callback. It receives the selected items and replaces the default trigger render whenever something is selected; the placeholder still shows when nothing is selected.
