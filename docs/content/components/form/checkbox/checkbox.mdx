...
- [SelectList](/components/collection/selectlist): A select list is an alternative to a checkbox when you need more than just a text label to represent options.
- [TagGroup](/components/collection/tag): A tag group is an alternative to a checkbox when you want to visually highlight selected options as individual tags.
