...
- **Checkbox input:** The clickable box showing the unchecked, checked, or indeterminate state.
- **Checkbox label:** Describes the option that the checkbox toggles.
...
<GuidelineTiles>
  <Do>
    ...
    <DoDescription>If the label is long, wrap to a second line.</DoDescription>
  </Do>
  <Dont>
    <DontFigure>
      <ImageZoom
...
Use checkboxes when there are no more than 10 to 15 options, as too many of them can overwhelm users and clutter the interface. For larger sets of options, refer to the [Multiple Selection Pattern](/patterns/user-input/multiple-selection) for a better user experience.
...
<Callout title="Auto-Expand on Selection">
...
To mark a checkbox with a `<Badge>`, such as an access level or a "New" flag, pass it to `badge` rather than building the decoration into `label` yourself. The slot holds it to the height of the label's line and sizes it to fit, so the box stays centred on the text instead of drifting above it, and a label that wraps still keeps the box on its first line.

The badge joins the checkbox's accessible name, announced right after the label text. That's right for an access-level mark, but a purely decorative badge should get `aria-hidden` instead.

<ComponentDemo name="checkbox-badge" />

### Error

Set `error` to mark the checkbox as invalid and pass an `errorMessage` to explain what needs to change. The error message replaces the description while `error` is set, and it is announced to screen readers via `aria-describedby`.

<ComponentDemo name="checkbox-error" />

## Props

<StorybookHintMessage component={'Checkbox'} />

### Checkbox

<AutoTypeTable path="Checkbox" name="CheckboxProps" />

### Checkbox.Group

<AutoTypeTable path="Checkbox/CheckboxGroup" name="CheckboxGroupProps" />
...
- [Switch](/components/form/switch): When there is only on option that should have an immediate effect.
...
- [SelectList](/components/form/selectlist): When you need more than just a text label to represent options, a `<SelectList>` can be used instead.
