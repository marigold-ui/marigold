...
To mark a checkbox with a `<Badge>`, such as an access level or a "New" flag, pass it to `labelAdornment` rather than building the decoration into `label` yourself. The slot holds it to the height of the label's line, so the box stays centred on the text instead of drifting above it, and a label that wraps still keeps the box on its first line. Use `<Badge size="inline">`, which is drawn to fit a line of text.
