...
Use two separate date fields when only the endpoints matter on their own. Open ended filters such as everything after a date leave one end empty, unrelated dates such as when a document was issued and when it expires carry no real span between them, and dates many years apart are faster to type than to navigate. In those cases a single [DatePicker](/components/form/datepicker) or two independent fields communicate the intent more honestly.
...
In reporting and analytics people reach for the same spans again and again, such as the last seven days or the current month. Offer them as buttons that set the `value` directly, so users fill the field in one click and can still fine tune it on the calendar.
...
Some ranges need a time as well as a date, such as an event that starts and ends at a set hour. Set `granularity` to `hour` or `minute` and each end of the range gains time segments.

<ComponentDemo name="date-range-picker-time" />
