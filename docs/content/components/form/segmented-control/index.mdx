...
The control fills the width of its container and scrolls horizontally when the segments do not fit, so it adapts to the space it is given — a card, a sidebar, or a narrow panel — instead of overflowing. By default the segments keep their natural size and sit at the start of the track. Set `width="full"` to stretch the segments so they divide the available width equally instead.
