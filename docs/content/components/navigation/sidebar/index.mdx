...
<Callout>
  The sidebar does not have dedicated icon support yet. If your application
  needs icons alongside navigation labels, coordinate across all applications
  first to ensure consistency. For now, use text-only labels.
</Callout>
...
For custom matching logic, pass a predicate `(href, key) => boolean` instead of a string. For escape-hatch cases (pinned items, items without an `href`), the per-item `active` prop on `Sidebar.Item` still works as a local override and stacks with the resolved match.
