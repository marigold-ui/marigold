...
`errorKeys` is what puts a failed step on the stepper instead of in a toast that disappears after five seconds. Pass the keys of the steps whose validation failed, and hand the field-level messages to the [`<Form>`](/components/form/form) as `validationErrors` so each input explains its own problem.
...
Give a `<Stepper.Item>` an `href` and it renders as a real link, so middle-click, open in a new tab, and copy link all work. Without an `href`, the step renders as a button and only calls `onSelectionChange`.
