...
The `<Pagination>` component consists of a "previous" button, a maximum of seven page buttons, and a "next" button. Each page button is a text `<Button>` labelled with its respective page number. If the total number of pages exceeds seven, some pages will be hidden with an ellipsis ("…").

<PaginationAnatomy />

- **Previous button:** Moves one page back.
- **First page:** Jumps to the first page of the data set.
- **Page buttons:** Up to seven buttons for selecting a page directly.
- **Ellipsis:** Marks the pages that are hidden when the total exceeds seven.
- **Last page:** Jumps to the last page of the data set.
- **Next button:** Moves one page forward.
...
<GuidelineTiles>
  <Do>
    <DoDescription>
      Set a sensible initial value for the number of visible results. This
      default value should protect the user as well as the system from being
      overloaded.
    </DoDescription>
  </Do>
</GuidelineTiles>
...
<ComponentDemo name="pagination-no-data" />
...
<StorybookHintMessage component={'Pagination'} />

### Pagination

<AutoTypeTable path="Pagination" name="PaginationProps" />
...
<GuidelineTiles>
  <Do>
    ...
    <DoDescription>
      Make sure to adjust any aria-labels if localizing the visible labels of
      the <i>previous</i> and <i>next</i> buttons, the text of the results
      indicator, or the label of the quantity selector.
    </DoDescription>
  </Do>
</GuidelineTiles>
