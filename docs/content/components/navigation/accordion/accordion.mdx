...
With the multiselection mode you can collapse and hide the items as you want. For sure you can set one or more defaultExpandedKeys. Use a multi-select accordion when users need to view multiple sections simultaneously, such as comparing information or accessing related FAQs. It works best when content is short and users benefit from having multiple sections open at once without losing context.
