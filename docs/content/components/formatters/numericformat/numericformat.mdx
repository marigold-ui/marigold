...
Use the `tabular` prop when you need your numeric output to line up neatly in columns or tables. When tabular is enabled (the default), each digit is given exactly the same width, so rows of numbers stay perfectly aligned. This is especially helpful in data tables, financial reports, or any grid layout where vertical alignment of digits improves readability. If you don’t need this alignment—say, in running text or headlines—you can disable it by setting tabular={false}.
