...
The use of `<Stack>` is recommended if you have two or more elements that must be the same distance apart. For the space you have to use our supported [space values](../../foundations/token-overview#spacing).
...
<StorybookHintMessage component={'Stack'} />

### Stack

<AutoTypeTable path="Stack" name="StackProps" />
...
<TeaserList
  items={[
    ...
    },
    {
      title: 'Split',
      href: '../layout/split',
      caption: 'Adds space between two elements in a flex layout.',
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
