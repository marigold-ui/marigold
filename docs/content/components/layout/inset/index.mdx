...
In order to add equal spacing to all sides use the `space` property with a padding spacing token (`square-*`, `squish-*`, `stretch-*`) or a numeric scale value.
...
To set only horizontal or vertical spacing, or to define different values for each, use the `spaceX` property for horizontal spacing and the `spaceY` property for vertical spacing. These props accept inset spacing tokens (`inset-tight`, `inset-snug`, `inset-regular`, `inset-relaxed`, `inset-loose`), as well as numeric scale values.
