...
Because `<Container>` caps the width of its content, that content is often narrower than the space the container fills. The `alignItems` prop controls where the content sits within that space.

By default (`none`) it stays at the start, which suits a normal left-aligned reading column. Reach for `alignItems` when a container sits in a wide area and its content should not hug the left edge:

- `center` centers the content in the container, the common choice for hero sections or a centered article on a wide page.
- `right` pushes the content to the end, for the rarer case where it should sit against the right edge.

In short, `contentLength` decides how wide the reading column is, and `alignItems` decides where that column sits.
