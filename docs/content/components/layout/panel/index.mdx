...
`Panel.Header` is the layout wrapper that arranges title, description, and actions in a grid. Drop a [`<Title>`](/components/content/title), an optional [`<Description>`](/components/content/description), and any action primitives (`<ActionButton>`, `<ActionGroup>`, `<ActionMenu>`, `<LinkButton>`) directly inside it. The header configures slot context so each piece picks up its level, id wiring, grid position, size, and variant from the Panel — consumers don't need to set any of that themselves.

Multiple actions belong in an `<ActionGroup>` so the cluster claims one grid cell and renders as a toolbar.
...
`<Title>` renders an `<h2>` by default. `<Panel.CollapsibleTitle>` renders one level deeper (`<h3>`) when the Panel has a title to nest under. In a Collapsible-only Panel (no `<Title>`), the CollapsibleTitle renders at the base `headingLevel` instead, since there's no parent heading to defer to.
