...
`<Panel.Title>` renders an `<h2>` by default. `<Panel.CollapsibleTitle>` renders one level deeper (`<h3>`) when the Panel has a title to nest under. In a Collapsible-only Panel (no Panel.Title), the CollapsibleTitle renders at the base `headingLevel` instead, since there's no parent heading to defer to.
