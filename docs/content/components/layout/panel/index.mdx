...
Panels live inside a [`<Page>`](/components/layout/page) under a [`<Page.Header>`](/components/layout/page#pageheader). Panel's default `headingLevel={2}` assumes a page `<h1>` above it — which `<Page.Header>` provides — so the outline reads `h1` (page) → `h2` (Panel) → `h3` (Panel collapsible) with no configuration.
