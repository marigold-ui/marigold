...
When aligning form inputs with buttons, you can use `alignY="bottom"` to ensure the button is visually aligned with the bottom edge of the input field. If a help text or error message appears below the input, the button will automatically adjust its alignment to stay visually consistent with the input.
