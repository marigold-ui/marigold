...
Use the `<Aside>` for building layouts were you need to have one dynamic main content with a fixed side content. When the screen becomes smaller, for example on mobile, the side content of the component will automatically wrap. You can also adjust this by setting the `wrap` property to a percentage value.
