...
The "surface" intent is a special case with distinct modifiers compared to the other background intents. Only the following modifiers can be used with it:
...
Border tokens are used to apply specific colors to the border of an element. They are equivalent to the border-color CSS rule or the [`border-{color}` utilities](https://tailwindcss.com/docs/border-color) in TailwindCSS.
...
Border tokens are used to apply specific colors to the border of an element. They are equivalent to the outline-color CSS rule or the [`outline-{color}` utilities](https://tailwindcss.com/docs/outline-color) in TailwindCSS.

#### Intents

Outline tokens are a special use case and should be used sparingly. Their usage is reserved for indicating keyboard navigation:
