...
Anything that read `scrollTop` from the old main container (or attached a scroll listener to it) needs to read from `window` / `document.documentElement` instead. A `height: 100%` on direct children of the main area is no longer constrained, so switch it to `min-h-dvh` or drop the constraint altogether.
...
**`role="alert"` replaced by an `announce` prop.** The previous implementation announced only the `error` variant, and did so by mounting `role="alert"` together with its content, which the WAI-ARIA spec warns against, and which made announcements unreliable on some screen reader and browser combinations. `<SectionMessage announce>` now routes the message text through react-aria's persistent live announcer (`polite` for `info` / `success` / `warning`, `assertive` for `error`). `announce` defaults to `true` for `variant="error"` and `false` otherwise, so the common case behaves as before. Re-announce the same message by passing a changing `key`.
...
`<Sidebar>` gains a second mode, and for full applications it is the new default: a persistent **rail** of icon-first top-level destinations next to a **panel** showing the active section's sub-navigation. A single column starts to strain once an app has several top-level sections that each carry their own sub-navigation. Everything either nests behind drill-downs, or the list grows past what users can scan. With `<Sidebar.Rail>`, both levels stay visible at once: switching sections is one click, and users never lose sight of where they are.
...
The shell adapts on its own. When `<AppShell>` contains a rail, it switches to a full-width top bar (pure CSS via `:has()`), so the brand never moves when the panel collapses. Place the brand and the new `<Sidebar.Toggle variant="rail" />` in the top navigation's start slot. Keyboard and screen reader support is thorough: the rail and the section panel are separate navigation landmarks, arrow keys and Home/End move across the rail on top of its flat tab order, the current page announces `aria-current="page"` while its section ancestor announces `aria-current="true"`, and the section panel's tab stop re-syncs to the current page when the route changes, so Tab re-enters at the active item. Collapsed, icon-only tiles reveal their label as a tooltip on hover or focus.

For theming, the shell's shared measurements move into tokens: `--spacing-topbar` is the vertical datum shared by the top bar, the sidebar brand row, and the rail's sticky offset, `--spacing-rail` / `--spacing-rail-collapsed` / `--spacing-rail-panel` size the rail columns, and `--spacing-touch-target` guarantees the 44px minimum row height on small screens. Two related shell refinements land alongside: the `AppShell` header row is now sized `auto` instead of a fixed `3.5rem`, so a shell without a `<TopNavigation>` no longer reserves an empty header band, and the shell's viewport-height claims read a new `--ui-viewport-height` custom property (with a `100dvh` fallback), so you can render the whole shell inside a bounded container such as an embedded preview or demo.
...
The same cascade drives `<ActionBar>` (see the breaking change above), `<SelectList.Option>` (a trailing in-row action reads as low-emphasis chrome automatically), and even `<TagGroup>`'s internal "Remove all" action. `<ActionMenu>`'s trigger participates too: it renders `secondary` on its own and `ghost` inside a header, option row, or group.
...
The callback also receives a second `details` argument with the selection `count`, which makes it easy to summarise a multi-select trigger instead of listing every value:
...
`renderValue` now also works when options are provided as static `<Select.Option>` children (previously it was silently skipped unless the options came from the `items` prop).
...
- **`<ProgressCircle>` and `<Loader>` accessible names.** A consumer-provided `aria-label` / `aria-labelledby` is respected instead of being overwritten by the generic localized "loading" message (which remains the fallback), and a fullscreen `<Loader>` with a custom `aria-labelledby` names its overlay dialog correctly. Named `size` tokens on `<ProgressCircle>` also resolve to valid numeric SVG dimensions now instead of emitting `width="defaultpx"` console errors.
...
- **`marigold doctor`.** A read-only command that diagnoses a project's Marigold setup: package presence, version match between `@marigold/components` and `@marigold/system`, freshness against the latest release, that `MarigoldProvider` actually wraps the app and its `theme` prop resolves to a real binding, Tailwind config, and React peer deps. It prints actionable fixes grouped by severity, supports `--format text|json` and `--offline`, and exits non-zero only on deterministic errors, so it is safe to gate CI on and easy for agents to consume. The CLI also now runs correctly when invoked through a symlinked global bin.
...
- **New Pick pattern and working example.** Picking is the counterpart to filtering: filtering narrows a view in place, picking finds records in a collection and commits them as a set. The new [Pick pattern](/patterns/user-input/pick) documents that distinction, a surface spectrum that scales with collection size (an inline multi-select, a searchable `<TagField>`, a `<Dialog>`, and a routed page as the exception), and the load-bearing practices: staged selections survive while search and filters narrow the list, the commit button bounds the selection and names the outcome, and the staged set stays visible as a removable `<Tag.Group>` rail. Four inline demos ship with the page, plus a complete example at `/examples/pick` where a fullscreen dialog picks from about fifty venues into a report that owns the committed set.
...
The app shell around it separates its regions on whitespace, and the structural lines that remain are plain, always-on borders: the sidebar divider, the sidebar header's edge, and the `TopNavigation` bottom border. An earlier iteration revealed those hairlines only while content scrolled underneath, but the scroll-driven seams did not earn their complexity in the shell and are gone (the Dialog header seam below stays, where the scroll signal actually carries information). The sidebar toggle icon also loses its springy overshoot and settles on the theme's standard `ease-out-quint` motion. And this is only the single-column story: the sidebar's new two-level rail mode is covered under _Components_ above.
...
On top of that, a long Dialog now signals that its body scrolls: the header is borderless at rest and fades in a bottom hairline once content scrolls beneath it, matching the sidebar's scroll-revealed seam. The mechanism is promoted to a reusable `ui-scroll-seam-*` primitive, and browsers without scroll-driven animations fall back to an always-on hairline.
