...
The `<ActionBar>` has been significantly enhanced with enter/exit animations, keyboard support (press Escape to clear selection), and built-in `<Table>` integration. The new `actionBar` render prop on `<Table>` automatically wires up selection state and positions the ActionBar relative to the table.
...
The active tab underline now slides smoothly between items with an animated transition. This works automatically and requires no code changes.
