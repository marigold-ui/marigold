...
`<Panel.Header>` is a single slot-configuration boundary: it publishes the heading level, ids, grid-area positioning, and the action size/variant cascade. Drop the slot-aware role primitives (`<Title>`, `<Description>`, `<ActionButton>`, `<ActionGroup>`, `<ActionMenu>`, `<LinkButton>`) directly inside it — no wrapper sub-components needed.
