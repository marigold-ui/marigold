...
Every design system reaches a point where the layers it has accumulated need to either be cleaned up or they start to drag everything down with them. v18.0.0 is where we stop layering and start reshaping. A new palette, a new set of semantic names, `<Multiselect>` finally retired, the last `util-surface-*` utilities removed, and React 19 as the baseline for every component.

This is the biggest single release we've shipped, and it's the first step in a longer effort -- not the last. The design language is evolving to feel more mature, read more clearly, and fit the real complexity of the product surfaces we support. Expect more token work and more refinements across the next few minor releases. v18 is the foundation you'll build on; it is not the destination.
...
Previously the theme exposed **two** parallel sets of tokens for status colors -- a "solid" variant (`--color-warning`, `--color-warning-foreground`) meant for icons on accent fills, and a "muted" variant (`--color-warning-muted`, `--color-warning-muted-foreground`, `--color-warning-muted-accent`) used for almost everything else (badges, banners, section messages). In practice, the muted set was the one you reached for 95% of the time, and the solid set caused more confusion than value.
...
- **`ui-state-hover`** -- the standard hover for list items, table rows, and menu items on white surfaces. Resolves to `bg-hover text-foreground`.
- **`ui-state-hover-ghost`** -- translucent hover for ghost buttons, tabs, and action bar buttons that need to adapt to whatever surface they sit on. Resolves to `bg-current/10`.
...
### `<Multiselect>` removed -- use `<TagField>`
