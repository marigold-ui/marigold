...
We've standardized how labels are defined across form components. Previously, `<Checkbox>` and `<Switch>` used children to define labels. To align with other form controls and provide clearer structure, they now use a label prop. The children prop is no longer supported.
...
A dedicated `label` prop was added to the `<Checkbox>` component to set the label instead of using `children`.
...
New drawer components have been added, although there is no documentation yet, and they are still in beta.
These components will be used in the upcoming filter pattern.
You can take a look at the drawer components in Storybook (https://marigold-latest.vercel.app/?path=%2Fstory%2Fcomponents-drawer--basic&globals=theme%3Arui).
The drawer components are only available in the RUI theme.
If you would like to use the drawer components, let us know and we can help you.

#### Form

An `unstyled` prop was added to the `<Form>` component to prevent layouts from breaking.
A `maxWidth` prop was added to make it easier to limit the width of all child controls.

#### Pagination

Keyboard navigation has been improved for the pagination component.
The documentation was improved by replacing images with code and interactive examples.
...
The component now allows formatting ranges; see the documentation for more information.
See link: /components/formatters/numericformat#formatting-ranges

#### Switch

A dedicated `label` prop was added to the `<Switch>` component to set the label instead of using `children`.
...
The `<Tag>` component can now be used in forms and behaves like a list of checkboxes.
The `<Tag>` component now supports the disabled state.
