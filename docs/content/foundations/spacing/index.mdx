...
`--bleed-px` is the one you want when content sits edge-to-edge and needs to line back up with the container's title. Any content area rendered with `bleed` publishes it — `Panel.Content`, `Panel.CollapsibleContent`, `Card.Content` and `Drawer.Content` — always holding that container's own horizontal padding.

Prefer it over the container-specific `--panel-px` and `--card-px`: it is scoped to the bled element rather than inherited from a root, so it is only set where edge alignment actually applies, and the same declaration works whichever container the content ends up in.
