...
Touch input needs more than the mouse minimum: list rows that are pointer-sized on desktop grow to the `touch-target` height token (`--spacing-touch-target`, 44px — the platform guideline for finger targets) on small screens, via `max-sm:min-h-touch-target`. Menu and ListBox options (which open in a [Tray](/components/overlay/tray) on mobile) and the sidebar drawer's nav rows use this token.
