...
<TokenTable
  tokens={[
    {
      name: 'background',
      value: 'oklch(0.945 0.004 54)',
      useFor:
        'Page-level background (<body>), a custom literal deepened off the charcoal scale so white surfaces separate by fill',
    },
...
A single shadow marks the one layer that floats above the page. Everything else in normal flow separates by fill and boundary instead: cards and panels by a deepened page ground, buttons by a raised cap (`ui-soft`, `ui-contrast`), fields by a flat well (`ui-control`).
