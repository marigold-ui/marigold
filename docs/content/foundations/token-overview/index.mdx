...
- `focus-highlight-bold`: uses `text-foreground` (the stronger tint still meets contrast with regular text)
...
The RUI theme is built on a warm neutral palette called **charcoal**, an oklch gray with a subtle warm hue. The 11-step scale runs from near-white (50) to near-black (950). All semantic color tokens below reference values from this palette.
  ...
  tokens={[
    {
      ...
      useFor:
        'Visible boundary of a bordered control (inputs, selects), applied via ui-control. Denser than surface-border and above the contrast threshold, so the edge stays visible. The three control-edge steps are derived from it',
    },
    {
      name: 'control',
...
<Callout title="Not a default for all controls">
  ...
  `border-control-edge` for checkboxes, radios and selection marks, a step
  derived from `control-border` and documented with the other edges under
...
<TokenTable
  tokens={[
    {
      name: 'soft',
      value: 'oklch(from charcoal-950 0.99 0.002 h)',
      useFor:
        'Resting fill for the soft cap (secondary Button, Menu trigger). Lightness and chroma are tuned by eye; the hue is derived from the palette',
    },
    {
      name: 'soft-hover',
      value: 'charcoal-100',
...
<TokenTable
  tokens={[
    ...
    },
    {
      ...
      value: 'charcoal-300',
      useFor: 'Strong focus background for inline segments (DateField)',
    },
  ]}
/>
...
<TokenTable
  tokens={[
    ...
    },
    {
      ...
      value: 'charcoal-300',
      useFor:
        'Boundary of a disabled control that paints its own edge, such as a checkbox or a switch: a soft, receding edge',
    },
  ]}
/>
...
The `ui-state-disabled` utility bundles background, text, border and cursor into one class, and it does not read these tokens. It derives all three colors from the element's own text color instead, so a disabled button or input recedes the same way on a white panel, the page background or a dark surface like the ActionBar. Reach for the tokens directly only when a control paints its disabled look itself, as Checkbox, Radio, Switch and Slider do.
...
Every boundary token in one place. Marigold draws element edges with eight tokens, and each one answers a different question. There is the decorative rim of a surface, the visible edge of a control, that edge dimmed for a disabled control, the three steps a small boolean control draws, the structural divider, and the focus ring. `surface-border` also appears in the [Surfaces](#surfaces) section, `control-border` in the [Control](#control) section, and `disabled-border` in the [Disabled](#disabled) section, and they are repeated here so the whole set is discoverable at a glance. See [Form Fields](/foundations/form-fields) for component-level guidelines and [Elevation](/foundations/elevation#surfaces-and-controls) for the surface and control rule these tokens implement.
...
<TokenTable
  tokens={[
    ...
    },
    {
      ...
      value: 'charcoal-950 / 26%',
      useFor:
        'The boundary of a control such as inputs, selects, and date fields, applied via ui-control. An affordance the user must see (WCAG 1.4.11). The three control-edge steps below are derived from it, so retuning this retunes them',
    },
    {
      name: 'control-edge',
      value: 'control-border +0.06 alpha',
      useFor:
        'The box edge of a small boolean control: Checkbox, Radio, and the selection mark a SelectList option or ListView row draws. One token so all four are identical. 2.15:1, the weight the opaque edge it replaced used to carry',
    },
    {
      name: 'control-edge-hover',
      value: 'control-border +0.18 alpha',
      useFor:
        "The hover step for that edge, 3.06:1. Matches ToggleButton's hover step rather than its value, because this edge idles denser",
    },
    {
      name: 'control-edge-disabled',
      value: 'control-border -0.06 alpha',
      useFor:
        'That edge dimmed, 1.57:1. Translucent rather than a palette rung because a disabled mark lands on surface, on the page ground and on the selected fill of a picked row, and an opaque value matches one of those exactly and disappears into it',
    },
    {
      name: 'disabled-border',
      value: 'charcoal-300',
    ...
    {
      name: 'border',
      value: 'charcoal-950 / 14%',
      useFor:
        'Structural lines drawn on a known surface, such as dividers, table row and grid lines, section rules, and internal separators. Translucent, so it holds the same ~1.36:1 contrast on every ground it is used on rather than varying with the fill behind it',
    },
    {
      name: 'ring',
      value: 'charcoal-600',
...
<Callout title="Which border token to pick for the job">
  ...
  **decorative** rim of a raised surface (Card, Panel, Dialog). It sits below
  ...
  vanishing, and it is the rung the `ui-control` family uses. The three
  `control-edge` steps are the same job for a **small boolean control**, where
  the box is tiny and the edge is the whole affordance: they stay translucent in
  every state, including disabled, because such a mark also has to hold its
  weight on the `selected` fill of a picked row. `border` draws **structural
  lines** such as dividers, grid lines, and section rules. It is translucent for
  the same reason, so a divider carries the same weight on a white surface as it
  does on the page ground. `ring` is the **focus** outline, the perceivable
