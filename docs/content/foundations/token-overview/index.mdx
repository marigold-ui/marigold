...
Selection tells users which item is currently active or focused. Not every selection looks the same though. Binary controls like checkboxes and switches flip between on and off, so they use `bg-primary` with `text-primary-foreground` for a strong, inverted style that clearly communicates the on/off state. List-based components like ListBox and Sidebar highlight one item among many, so they use a subtler `bg-selected` tint. A separate `focus-highlight` token handles keyboard navigation, marking the item the user is about to select.
...
**Keyboard focus** marks the item the user is navigating to, before confirming a selection:
...
<GuidelineTiles>
  <Do>
    <DoDescription>
      ...
      `bg-primary` for binary controls (Checkbox, Switch, ToggleButton).
    </DoDescription>
  </Do>
  <Dont>
    <DontDescription>
      Don't use `bg-selected` for binary controls. A checkbox or switch should
      use the strong `bg-primary` pattern to clearly communicate its on/off
      state.
    </DontDescription>
  </Dont>
</GuidelineTiles>
