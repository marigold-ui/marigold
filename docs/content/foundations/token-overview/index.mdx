...
Raised surfaces (Card, Panel, Dialog, Menu) draw their rim with `surface-border`, a translucent decorative hairline applied through the `ui-surface` utility rather than a standalone border. See [Elevation](/foundations/elevation) for how it combines with the shadow tiers.
...
<Callout title="Not a default for all controls">
  ...
  border: the `ui-surface` ring for inputs and selects, `border-border` for
  checkboxes and radios. Only reach for `bg-control` when a control communicates
  ...
  tokens={[
    {
      ...
      value: 'charcoal-300',
      useFor:
        'Selection wash for list items (SelectList, ListBox, Sidebar) — one step darker than hover — paired with one ink mark: the row indicator in option lists, the inset-shadow-selection stroke in navigation',
    },
    {
      name: 'selected-bold',
      value: 'charcoal-900',
...
**Navigation** (Sidebar), where rows have no indicator, marks the current item with the stroke:

```tsx
<a className="data-active:bg-selected data-active:inset-shadow-selection" />
```
  ...
  tokens={[
    {
      ...
      useFor:
        'Functional edges: any boundary that carries meaning or is an affordance, so it must stay perceivable (WCAG 1.4.11). Covers form-control boundaries (Input, Select, TextArea, Checkbox, Radio) and opaque lines on a known surface (dividers, table row and grid lines)',
    },
    {
      name: 'ring',
      value: 'charcoal-600',
...
<Callout title="Two border tokens: pick by the job">
  Use `border` for **functional** edges. It is opaque and stays perceivable
  (WCAG 1.4.11), so it covers every form-control boundary and overlapping lines
  like grid cells and dividers. Controls are surfaces, but their edge is an
  affordance, so `ui-input` overrides the surface rim with `border` (Checkbox
  and Radio use it directly). Use `surface-border` for the **decorative** rim of
  a raised surface. Being translucent it sits below the contrast threshold, so
  never use it where the edge has to be seen.
</Callout>
