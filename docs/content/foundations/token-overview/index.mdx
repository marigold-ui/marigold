...
The three boundary tokens, one per job: a control's visible edge, structural lines drawn on a surface, and the focus ring. `border` is not a form-control token — it draws dividers and grid lines on any surface. See [Form Fields](/foundations/form-fields) for component-level guidelines and [Elevation](/foundations/elevation#surfaces-and-controls) for the surface/control rule these tokens implement.
...
<Callout title="Three border tokens: pick by the job">
  ...
  `surface-border` at about 2.5 times the density, so it stays clearly visible
  and adapts to the ground beneath it. `border` draws **structural lines** such
  as dividers, grid lines, and section rules. It is opaque on purpose, because
  separators are many strokes drawn by many cells, and translucent strokes would
  render darker wherever they overlap or cross. `surface-border` is the
