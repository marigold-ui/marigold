...
Most form controls sit on a white surface and use borders to define their shape: a text input, a select dropdown, a checkbox outline. But some controls communicate their state through a **filled background** instead of a border. The switch track is the clearest example: when OFF, the track needs to look like a solid, physical groove that the thumb can slide along. A white or transparent background would make the track disappear into the surface, while using `selected` or `disabled-surface` would send the wrong signal.
...
Every boundary token in one place. Marigold draws element edges with five tokens, each answering a different question: the decorative rim of a surface, the visible edge of a control, that edge dimmed for a disabled control, structural dividers, and the focus ring. `surface-border` and `disabled-border` also appear in the [Surfaces](#surfaces) and [Disabled](#disabled) sections beside their fills — they are repeated here so the whole set is discoverable at a glance. See [Form Fields](/foundations/form-fields) for component-level guidelines and [Elevation](/foundations/elevation#surfaces-and-controls) for the surface/control rule these tokens implement.
...
<TokenTable
  tokens={[
    ...
    },
    {
      ...
      useFor:
        'The boundary of a control: inputs, selects, date fields (via ui-control), checkboxes and radios (directly). An affordance the user must see (WCAG 1.4.11)',
    },
    {
      name: 'disabled-border',
      value: 'charcoal-300',
    ...
    },
    {
      ...
      useFor:
        'Structural lines drawn on a known surface: dividers, table row and grid lines, section rules, internal separators. Opaque so crossing strokes cannot stack',
    },
    {
      name: 'ring',
      value: 'charcoal-600',
...
<Callout title="Which border token: pick by the job">
  ...
  **decorative** rim of a raised surface (Card, Panel, Dialog); it sits below
  ...
  `control-border` marks an element you **operate** — the same translucent
