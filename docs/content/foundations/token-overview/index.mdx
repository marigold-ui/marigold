...
<Callout title="Not a default for all controls">
  Most form controls use `bg-surface` (white) with a `border-border` outline.
  Only reach for `bg-control` when a control communicates its state through a
  filled background rather than a border, like switch tracks, slider tracks, and
  similar "trough" elements.
</Callout>
...
<GuidelineTiles>
  ...
  <Dont>
    <DontDescription>
      ...
      selects, or textareas. Those should use `bg-surface` with `border-border`.
    </DontDescription>
  </Dont>
</GuidelineTiles>
...
Alongside the color tokens, the theme provides two mask-image variables that hold the glyph shapes for access marks: `--access-mask-lock` (master) and `--access-mask-key` (admin). The `ui-access-master` and `ui-access-admin` utilities consume them to render the glyph as themed generated content in the access foreground color. The glyph is purely decorative; on links and menu items the access level is exposed to assistive technology through a visually hidden text label rendered by the component. The glyphs are derived from the [lucide](https://lucide.dev) icons `lock-keyhole` and `key`.

The `master`/`admin` variants of `Badge`, `Link`, and `Menu.Item` build on these utilities. See the [Admin- and Mastermark pattern](/patterns/layout/admin-master-mark) for when and how to apply them.
