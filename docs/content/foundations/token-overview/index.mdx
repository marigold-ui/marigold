...
Raised surfaces (Card, Panel, Dialog, Menu) draw their rim with `surface-border`, a translucent decorative hairline applied through the `ui-surface` utility rather than a standalone border. Controls (inputs, selects, the secondary Button) use the same mechanism with the denser `control-border`. See [Borders](#borders) below, and [Elevation](/foundations/elevation) for how boundaries combine with the shadow tiers.
...
This section covers the `control` fill token. The visible edge that gives most controls their shape comes from a different utility, `ui-control`, which is documented with the other boundaries under [Borders](#borders) and explained in [Elevation](/foundations/elevation#surfaces-and-controls).

Most form controls sit on a white surface and use borders to define their shape, such as a text input, a select dropdown, or a checkbox outline. But some controls communicate their state through a **filled background** instead of a border. The switch track is the clearest example: when OFF, the track needs to look like a solid, physical groove that the thumb can slide along. A white or transparent background would make the track disappear into the surface, while using `selected` or `disabled-surface` would send the wrong signal.

`control` fills this gap. It provides a medium-weight neutral background that says: "I am an interactive control in my resting state." It pairs naturally with `selected-bold` for the ON state, creating a clear before/after contrast.
  ...
  tokens={[
    {
      ...
      value: 'charcoal-300',
      useFor:
        'Resting fill for controls that need a visible inactive state (e.g. switch track)',
    },
  ]}
/>
...
<Callout title="Not a default for all controls">
  ...
  border: the `ui-control` ring for inputs and selects, `border-control-border`
  for checkboxes and radios. That boundary is the `control-border` token,
  documented with the other edges under [Borders](#borders). Only reach for
  `bg-control` when a control communicates its state through a filled background
  rather than a border, like switch tracks, slider tracks, and similar "trough"
  elements.
</Callout>
...
Every boundary token in one place. Marigold draws element edges with five tokens, and each one answers a different question. There is the decorative rim of a surface, the visible edge of a control, that edge dimmed for a disabled control, the structural divider, and the focus ring. `surface-border` and `disabled-border` also appear in the [Surfaces](#surfaces) and [Disabled](#disabled) sections beside their fills, and they are repeated here so the whole set is discoverable at a glance. See [Form Fields](/foundations/form-fields) for component-level guidelines and [Elevation](/foundations/elevation#surfaces-and-controls) for the surface and control rule these tokens implement.
...
<Callout title="Which border token to pick for the job">
  ...
  **decorative** rim of a raised surface (Card, Panel, Dialog). It sits below
  ...
  charcoal stroke as `surface-border` at about 2.5 times the density, so it
  stays clearly visible and adapts to the ground beneath it. `disabled-border`
  is that control edge **dimmed** for an inactive control, receding rather than
