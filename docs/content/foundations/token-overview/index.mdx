...
Alongside the color tokens, the theme provides two mask-image variables that hold the glyph shapes for access marks: `--access-mask-lock` (master) and `--access-mask-key` (admin). They are consumed by the `ui-access-master` and `ui-access-admin` utilities, which render the glyph as themed generated content in the access foreground color and expose the restriction to assistive technology via CSS alternative text (`content: '' / 'Master'`). The glyphs are derived from the [lucide](https://lucide.dev) icons `lock-keyhole` and `key`.
