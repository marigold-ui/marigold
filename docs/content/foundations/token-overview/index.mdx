...
Some form controls need a filled resting state that is visually distinct from both the white surface they sit on and the stronger colors reserved for selection or interaction. The switch track is the primary example: in its OFF position the track must look like a tangible, inactive control rather than empty space, but it should not imply selection (`selected`) or error (`disabled-surface`).
  ...
  tokens={[
    {
      name: 'neutral',
      value: 'charcoal-300',
      useFor: 'Resting fill for form controls (e.g. switch track in OFF state)',
    },
  ]}
/>

```tsx
// Switch track: neutral when off, selected-bold when on
<div className="bg-neutral group-selected/switch:bg-selected-bold" />
```

<GuidelineTiles>
  <Do>
    <DoDescription>
      Use `bg-neutral` for the inactive/resting fill of controls that need a
      visible background, like the switch track.
    </DoDescription>
  </Do>
  <Dont>
    <DontDescription>
      Don't use `bg-neutral` for disabled states or content backgrounds. Use
      `bg-disabled-surface` for disabled controls and `bg-muted` for subtle
      content tints.
    </DontDescription>
  </Dont>
</GuidelineTiles>
