  ...
  tokens={[
    {
      ...
      value: 'charcoal-300',
      useFor:
        'Opaque lines on a known surface: dividers, table row and grid lines, binary-control outlines (Checkbox, Radio)',
    },
    {
      name: 'border-surface',
      value: 'charcoal-950 / 10%',
      useFor:
        'The edge of a raised surface, applied via the ui-surface utility (Input, Select, Card, Panel, Menu, …)',
    },
    {
      name: 'ring',
      value: 'charcoal-600',
...
<Callout title="Why two border tokens?">
  `border` is opaque; `border-surface` is the same charcoal hue at 10% alpha.
  The surface edge is translucent so it composites over whatever ground it sits
  on — staying consistent on white, the page background, or a tinted panel,
  where a fixed gray reads heavy on white and washes out on gray. `border` stays
  opaque because it is used for lines that **overlap** (grid-cell borders,
  stacked dividers), where translucent strokes would double-darken at every
  crossing. `border-surface` is drawn as the outset ring inside the `ui-surface`
  utility, not applied as a standalone border class — the outset placement is
  what lets it composite over the ground and keeps it from being covered by
  edge-to-edge child content.
</Callout>
