...
Explicit local props win over container-injected defaults where it matters most. `<ActionButton.Group>` documents this with three precedence rules. `size` cascades from the group, `variant` is overridden by the local prop (so a destructive button stays destructive inside a ghost group), and `disabled` is overridden by the local prop with the group as the fallback default.

<Callout title="Where to learn more">
  For the deep dive into the rule about which contexts to reuse, the recipe for
  building a new slot-configurable primitive, and a hand-built example
  container, see the [Slot
  Configuration](/patterns/composition/slot-configuration) pattern page.
</Callout>
