...
---
title: MyComponent
badge: alpha # or beta, or omit for stable
---
```
...
Use `toHaveClass` for specific style assertions, or `toMatchInlineSnapshot` for className testing:

```tsx
test('accepts a variant with parts', () => {
  render(<SectionMessage data-testid="messages" variant="warning" />);
...
chore[deps]: update dependency @tanstack/react-query to v5.90.20
docs: Fix typo in `Style.stories.tsx`
test: Use `userEvent` instead of `fireEvent` to make tests more stable
[DST-1187]: Migrate test files to story-based approach
```
