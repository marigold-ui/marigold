...
The used token is `bg-surface-overlay` and should be used together with `shadow-surface-overlay` wihch creates a shadow to display a higher hierarchy.
...
In this example all elevation levels are shown in a full page example layout. The default surface is the base, with the sunken and raised surfaces for creating a visual hierarchy. Overlay surface will be on elements like `Popover`.
