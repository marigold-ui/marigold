...
- **Tight** - Refers to packed containers side-by-side without any wasted space, like a dense list of data cells or entries in a table. It communicates high-density scanning where maximum information is prioritized over breathing room. _Common patterns: Data table rows, multi-line card metadata, Headline + Description, ..._
- **Related** - The small gap communicates that two distinct elements form a single conceptual pair or unit. Use it for elements that belong together but are each self-contained, such as paired form fields (e.g. Start Date + End Date, Postal Code + City) or an icon next to a text label. _Common patterns: Paired form fields, Icon + Text, Headline + Subhead, ..._
- **Regular** - The wider, balanced space indicates two individual, self-contained elements standing as equals within the same content flow. This is the baseline rhythm for form fields and the page's standard reading rhythm. _Common patterns: Baseline form field spacing, list items, paragraphs, ..._
- **Group** - A substantial gap that visually clusters related elements into a sub-group within a larger context. It separates a block of content from its surroundings, signaling that the enclosed elements belong together as a logical unit. _Common patterns: Cards in a grid, address block within a form, content clusters in a sidebar, ..._
