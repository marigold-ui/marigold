...
export const SelectList: ThemeComponent<'SelectList'> = {
  container: cva({
    base: [
      ...
      // 40rem (640px), the surface fills its parent so the stacked layout reads
      // as a regular vertical list.
  ...
  list: cva({
    base: [
      ...
      // than 40rem (640px). Keyboard navigation still works on both axes thanks
      // to `layout="grid"` on the underlying GridList.
