...
The fallback stands down when a `slot` is set. That guard is load-bearing, not defensive tidiness. React Aria's `TagGroup` names its `remove` slot through context, and a locally set `aria-label` wins over a context one, so an unconditional fallback relabelled every removable tag's button from "Remove News" to "Close". A test in `TagGroup.test.tsx` now pins that behaviour.
