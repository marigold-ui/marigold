...
New props on `<TagGroup>`: `error`, `required`, `disabled`, `validate`, `validationBehavior`, `form`. Public API normalised to Marigold's convention — `isInvalid` / `isRequired` / `isDisabled` are removed and `onSelectionChange` is renamed to `onChange`. `selectionMode` now defaults to `'multiple'`.
