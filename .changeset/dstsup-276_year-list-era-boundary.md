...
The window is now anchored at year 1 rather than centred whenever the focused year sits too close to the boundary to centre it. `Calendar` and `RangeCalendar` both keep a 41-year list: AD 5 offers `1 … 41`, as does AD 1. Since react-aria only shifts its window for `minValue`, which this component does not own, the window is instead sized to guarantee 41 in-era years and the entries that rolled into the previous era are dropped.
