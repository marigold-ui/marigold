...
Also renames the internal `--panel-accent` CSS variable to `--panel-text`: it holds the foreground (text) color, not an accent color, so the old name was misleading. Pure rename — no visual or behavioural change. Mirrors the `--card-text` rename in DST-1373.
