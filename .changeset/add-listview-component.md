...
A row is a two-column grid: the text stack, then the row's trailing controls. Row content is authored with Marigold's dedicated slot-aware components (`<TextValue>`, `<Description>`, `<Title>`), never a raw `<Text slot="…">`, and each of them claims its cell through slot context — so text still lands correctly when it's wrapped in a fragment, behind `memo()`, or inside a consumer's own component. A bare string child is wrapped as the row's text value. Trailing controls claim their cell through Marigold's `ButtonContext` and inherit the row's `ghost` cascade, the same mechanism `<Panel.Header>` and `<SelectList.Option>` use; a row with more than one control wraps them in a `<ButtonGroup>`. `<ListView.Item>` forces any nested `<Title>` to render as a `<span>` instead of a real heading, so a list of rows never emits one document heading per row.
