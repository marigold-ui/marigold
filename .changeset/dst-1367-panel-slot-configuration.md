...
`<Title>` gains an `as` prop (also configurable via slot context) for rendering the title as a non-heading element when the surrounding container already provides heading semantics — for example, inside a Disclosure trigger button. Default behaviour is unchanged.
