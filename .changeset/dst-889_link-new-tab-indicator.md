...
A `<Link>` with `target="_blank"` now appends an external-link icon after its label plus a visually hidden, localized "opens in a new tab" warning (WCAG G201), and defaults `rel` to `noopener`. A `rel` you pass yourself still wins.
